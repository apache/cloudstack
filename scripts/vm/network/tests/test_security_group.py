...
def program_l3_host(order, with_rules=True):
...
class TestDirectRoutedRules(unittest.TestCase):
    ...
    def test_from_instance_never_accepts(self):
        """ A from-Instance allow must not end evaluation: the destination Instance's ingress
        rules still have to run. Allowed traffic returns, and nothing after the egress jump may
        drop it. """
        for ipt in ['iptables', 'ip6tables']:
            chain = [c for c in self.captured if c.startswith(ipt + ' -A i-2-7-def ')]
            from_vm = [c for c in chain if '--physdev-in' in c]
            self.assertTrue(from_vm)
            for cmd in from_vm:
                self.assertNotIn('-j ACCEPT', cmd, cmd)
            egress = [c for c in chain if c.endswith('-j i-2-7-VM-eg')]
            self.assertEqual(1, len(egress))
            for cmd in chain[chain.index(egress[0]) + 1:]:
                self.assertNotIn('--physdev-in', cmd, cmd)
        self.assertIn("ip6tables -A i-2-7-def -m physdev --physdev-in vnet3 -p icmpv6 --icmpv6-type neighbor-solicitation -m hl --hl-eq 255 -j RETURN", self.captured)
        self.assertIn("iptables -A i-2-7-def -m physdev --physdev-in vnet3 -m state --state ESTABLISHED,RELATED -j RETURN", self.captured)

    def test_egress_denied_until_rules_arrive(self):
        # Allowed egress returns, so an empty egress chain would allow everything
        self.assertIn("iptables -A i-2-7-VM-eg -j DROP", self.captured)
        self.assertIn("ip6tables -A i-2-7-VM-eg -j DROP", self.captured)
...
class TestDirectRoutedEgressRules(unittest.TestCase):
    """ add_network_rules: egress rules on the L3 path return instead of accepting and the
    egress chain itself drops the rest; ingress rules accept as always. """
    ...
    def test_l3_egress_rules_return_and_the_chain_drops_the_rest(self):
        captured = self.capture(True, "E:tcp;80;80;10.1.1.0/24,fd00::/64,NEXT;I:tcp;22;22;0.0.0.0/0,NEXT;")
        self.assertIn("iptables -I i-2-7-VM-eg -p tcp -m tcp --dport 80:80 -d 10.1.1.0/24 -j RETURN", captured)
        self.assertIn("ip6tables -I i-2-7-VM-eg -p tcp -m tcp --dport 80:80 -d fd00::/64 -j RETURN", captured)
        self.assertIn("iptables -I i-2-7-VM -p tcp -m tcp --dport 22:22 -s 0.0.0.0/0 -j ACCEPT", captured)
        self.assertIn("iptables -A i-2-7-VM-eg -j DROP", captured)
        self.assertIn("ip6tables -A i-2-7-VM-eg -j DROP", captured)
        for cmd in captured:
            if 'i-2-7-VM-eg' in cmd:
                self.assertNotIn('ACCEPT', cmd, cmd)

    def test_l3_without_egress_rules_returns_everything(self):
        captured = self.capture(True, "I:tcp;22;22;0.0.0.0/0,NEXT;")
        self.assertIn("iptables -A i-2-7-VM-eg -j RETURN", captured)
        self.assertIn("ip6tables -A i-2-7-VM-eg -j RETURN", captured)
...
class TestSharedFrameworkHelpers(unittest.TestCase):
    ...
    def test_both_frameworks_share_the_helpers(self):
        ...
        # the L3 framework hooks FORWARD into the shared chains; the classic one gates on
        # physdev-is-bridged per bridge
        self.assertIn("iptables -I FORWARD -j BF-L3-IN", l3)
        self.assertIn("iptables -I FORWARD -j BF-L3-OUT", l3)
        for cmd in l3:
            if 'FORWARD' in cmd:
...
class TestDirectRoutedFramework(unittest.TestCase):
    ...
    def check_structure(self, model, bridges):
        for ipt in ['iptables', 'ip6tables']:
            t = model.tables[ipt]
            self.assertEqual(t['FORWARD'][:2], ["-j BF-L3-IN", "-j BF-L3-OUT"])
            self.assertEqual(t['FORWARD'].count("-j BF-L3-IN"), 1)
            self.assertEqual(t['FORWARD'].count("-j BF-L3-OUT"), 1)
            for br in bridges:
                self.assertIn("-i %s -j BF-%s-IN" % (br, br), t['BF-L3-IN'])
                jump_out = "-o %s -j BF-%s-OUT" % (br, br)
                drop_out = "-o %s -j DROP" % br
                self.assertLess(t['BF-L3-OUT'].index(jump_out), t['BF-L3-OUT'].index(drop_out))
                self.assertGreater(t['FORWARD'].index("-i %s -j ACCEPT" % br), 1)
            self.assertEqual(len(t['BF-L3-IN']), len(bridges))
    ...
    def test_missing_hook_restores_both_in_order(self):
        sg, model = self.build(["brdr-42"])
        for ipt in ['iptables', 'ip6tables']:
            model.tables[ipt]['FORWARD'].remove("-j BF-L3-OUT")
            model.tables[ipt]['FORWARD'].insert(0, "-o cloudbr0 -j DROP")
        sg.add_l3_fw_framework("brdr-43")
        self.check_structure(model, ["brdr-42", "brdr-43"])
        self.assertEqual(model.tables['iptables']['FORWARD'][2], "-o cloudbr0 -j DROP")

    def test_missing_backstop_is_appended_below_the_jump(self):
        sg, model = self.build(["brdr-42", "brdr-43"])
        model.tables['iptables']['BF-L3-OUT'].remove("-o brdr-42 -j DROP")
        model.tables['iptables']['BF-L3-OUT'].remove("-o brdr-42 -j BF-brdr-42-OUT")
        model.tables['iptables']['BF-L3-OUT'].append("-o brdr-42 -j DROP")
        sg.add_l3_fw_framework("brdr-42")
        self.check_structure(model, ["brdr-42", "brdr-43"])
...
class TestDirectRoutedTraversal(unittest.TestCase):
    ...
    def test_instance_to_fabric(self):
        def check(m, ctx):
            ...
            # replies to connections opened towards the Instance pass without an egress rule
            p = pkt(A, dst="8.8.8.8", dport=51000)
            p['established'] = True
            self.assertEqual(m.forward(p), 'ACCEPT', ctx)
        self.for_both_orders(check)

    def test_egress_denied_before_rules_arrive(self):
        for order in ((A, B), (B, A)):
            _, m = program_l3_host(order, with_rules=False)
            self.assertEqual(m.forward(pkt(A, dst="8.8.8.8", dport=80)), 'DROP')
            self.assertEqual(m.forward(pkt(B, dst="8.8.8.8", dport=80)), 'DROP')
            self.assertEqual(m.forward(pkt(A, B, dport=80)), 'DROP')
...
class TestDirectRoutedTeardown(unittest.TestCase):
    ...
    def test_destroying_an_instance_leaves_the_bridge_framework(self):
        sg, model = program_l3_host((A, B))
        sg.destroy_network_rules_for_vm(A['name'], A['vif'])
        for ipt in ['iptables', 'ip6tables']:
            t = model.tables[ipt]
            for chain in ['i-2-7-def', 'i-2-7-VM', 'i-2-7-VM-eg']:
                self.assertNotIn(chain, t)
            self.assertFalse(any('i-2-7' in r for rules in t.values() for r in rules))
            # the bridge stays until the Agent deletes it, and so do its rules
            self.assertEqual(t['BF-brdr-43-IN'], [])
    ...
    def test_cleanup_rules_removes_the_rules_of_a_deleted_bridge(self):
        sg, model = program_l3_host((A, B))
        sg.virshlist = lambda states: [A['name'], B['name']]
        with mock.patch('os.path.exists', side_effect=lambda p: p != "/sys/class/net/brdr-43"):
            sg.cleanup_rules()
        for ipt in ['iptables', 'ip6tables']:
            t = model.tables[ipt]
            for chain in ['BF-brdr-43', 'BF-brdr-43-IN', 'BF-brdr-43-OUT']:
                self.assertNotIn(chain, t)
            self.assertFalse(any('brdr-43' in r for rules in t.values() for r in rules))
            # the Instance on the other bridge, and the shared chains, are untouched
            self.assertEqual(t['FORWARD'], ["-j BF-L3-IN", "-j BF-L3-OUT", "-i brdr-42 -j ACCEPT"])
            self.assertIn("-i brdr-42 -j BF-brdr-42-IN", t['BF-L3-IN'])
            self.assertIn("-o brdr-42 -j DROP", t['BF-L3-OUT'])
            self.assertTrue(any('i-2-8-def' in r for r in t['BF-brdr-42-IN']))
        self.assertEqual(model.forward(pkt(None, B, src="10.1.1.50", dport=80)), 'ACCEPT')
        self.assertEqual(model.forward(pkt(B, dst="8.8.8.8", dport=22)), 'ACCEPT')
    ...
    def test_dispatch_deletion_is_anchored_on_the_chain_name(self):
        ...
        sg.delete_rules_for_vm_in_bridge_firewall_chain("i-2-7-VM")
        for ipt in ['iptables', 'ip6tables']:
            t = model.tables[ipt]
...
class TestVerifyRules(unittest.TestCase):
    ...
    def test_l3_expectations(self):
        ...
        self.assertIn("-A i-2-7-def -m physdev --physdev-in vnet3 -m state --state RELATED,ESTABLISHED -j RETURN", seen)
        self.assertNotIn("-A i-2-7-def -m physdev --physdev-in vnet3 -j DROP", seen)
        for rule in seen:
            self.assertNotIn('--physdev-is-bridged', rule, rule)
...
class TestIpv6OnlyDirectRouted(unittest.TestCase):
    ...
    def test_ipv4_from_the_instance_is_dropped(self):
