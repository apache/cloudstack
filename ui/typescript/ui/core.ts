...
(function ($, cloudStack) {
    ...
    var makeNavigation = function (args) {
        ...
        $.each(args.sections, function (sectionID, args) {
            ...
            if (args.customIcon) {
                $li.addClass('custom-icon').find('span.icon').html('').append($('<img>').attr({
                    src: args.customIcon
                }));
            }
    ...
    var pageElems = {
        header: function (args) {
            ...
            $('<div>').addClass('select default-view active')
                ...
                .prepend($('<span>').addClass('icon').html('&nbsp;')))
                .append(
            // Project View
            $('<div>').addClass('select project-view')
                ...
                if ($target.closest('.select.project-view').length) {
                    ...
                    $('.select.project-view').html('<span class="icon">&nbsp;</span>' + _l('label.project.view')).attr('title', '');
                    // Clear out project
            ...
            var $userInfo = $('<div>').attr({
                ...
                .append($('<div>').addClass('name').text(args.context && args.context.users ?
                cloudStack.concat(userLabel, 21) : 'Invalid User'))
                .append($('<div>').addClass('icon options')
                .append($('<div>').addClass('icon arrow')));
    ...
    $.fn.cloudStack = function (args) {
        ...
        $(['label.logout', 'label.help', 'label.about']).each(function () {
            ...
            if (this == 'label.about') {
                $link.addClass('about').click(function () {
                    var $logo = $('<div>').addClass('logo').text(_l('label.app.name')), $version = $('<div>').addClass('version').text(g_cloudstackversion), $about = $('<div>').addClass('about').append($logo).append($version);
        ...
        $.validator.addMethod("disallowSpecialCharacters", function (value, element) {
        ...
        if (!args.hasLogo)
            $('#header, #header .controls').addClass('nologo');
...
})(window.jQuery, window.cloudStack ? window.cloudStack : window.cloudStack = {});
