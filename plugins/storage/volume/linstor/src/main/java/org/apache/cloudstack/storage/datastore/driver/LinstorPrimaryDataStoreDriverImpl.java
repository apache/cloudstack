...
public class LinstorPrimaryDataStoreDriverImpl implements PrimaryDataStoreDriver {
    ...
    private String cloneResource(long csCloneId, VolumeInfo volumeInfo, StoragePoolVO storagePoolVO) {
        ...
        if (tmplPoolRef != null) {
            ...
            final String snapName = LinstorUtil.RSC_PREFIX + "clone-snap-" + RandomStringUtils.randomAlphabetic(6);
            final DevelopersApi linstorApi = LinstorUtil.getLinstorAPI(storagePoolVO.getHostAddress());

            try {
                Snapshot snap = new Snapshot();
                snap.setName(snapName);
                ApiCallRcList answers = linstorApi.resourceSnapshotCreate(cloneRes, snap);
                checkLinstorAnswersThrow(answers);
                ...
                try
                {
                    ...
                    answers = linstorApi.resourceDefinitionCreate(rdCreate);
                    checkLinstorAnswersThrow(answers);
                    ...
                    answers = linstorApi.resourceSnapshotDelete(cloneRes, snapName);
                    logLinstorAnswers(answers);
                }
            } catch (ApiException apiEx) {
                s_logger.error("Linstor: ApiEx - " + apiEx.getMessage());
