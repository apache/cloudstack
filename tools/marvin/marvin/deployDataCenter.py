...
"""
@Desc :
class DeployDataCenters: Deployes datacenters according to a json
                         configuration file
class DeleteDataCenter:  Deletes a datacenter based upon the dc cfg
                         settings provided.
                         This settings file is exported
...
class DeployDataCenters(object):
    ...
    def __cleanAndExit(self):
        ...
        self.__tcRunLogger.debug("===Deploy DC Failed, So Cleaning to Exit===")
        remove_dc_obj = DeleteDataCenter(self.__testClient,
        ...
        if remove_dc_obj:
            if remove_dc_obj.removeDataCenter() == FAILED:
                ...
                self.__tcRunLogger.debug("===Removing DataCenter Failed===")
            else:
    ...
    def createPods(self,
        ...
        for pod in pods:
            ...
            if createpodResponse != \
                    FAILED and createpodResponse.id:
    ...
    def createNetworks(self, networks, zoneId):
        ...
        for network in networks:
            ...
            if networkcmdresponse != \
                    FAILED and networkcmdresponse.id:
    ...
    def createPhysicalNetwork(self, net, zoneid):
        ...
        if phynetwrk != FAILED and phynetwrk.id:
            self.__tcRunLogger.\
            ...
            if pnetprovres and len(pnetprovres) > 0:
                if provider.name == 'VirtualRouter'\
                   or provider.name == 'VpcVirtualRouter':
                    ...
                    if self.__apiClient.\
                            configureInternalLoadBalancerElement(
                            internallbconfig) == FAILED:
                        self.__tcRunLogger.exception(
                            "==== Configure Internal LB Failed===")
                        self.__cleanAndExit()
                    self.enableProvider(pnetprovres[0].id)
                ...
                if result != FAILED and result.id:
                    ...
                    self.__tcRunLogger.\
                        exception("==== AddNetworkServiceProvider "
                                  "Failed=====")
                    self.__cleanAndExit()
        ...
        for zone in zones:
            ...
            if zone.networktype == "Basic":
                ...
                listnetworkofferingresponse = \
                    self.__apiClient.listNetworkOfferings(listnetworkoffering)
            ...
            if isPureAdvancedZone:
                ...
                if networkcmdresponse != \
                    ...
                    self.__tcRunLogger.\
                        exception("====Create Network Failed=====")
                    self.__cleanAndExit()
...
class DeleteDataCenter:

    '''
    @Desc : Deletes the Data Center using the settings provided.
            Reads the Configuration provided from file
            Uses the read configuration to delete the DataCenter
    '''
        ...
        if cmd_name == "deleteHostCmd":
            ...
            if host_maint_resp != FAILED:
                ...
                for i in xrange(retries):
                    ...
                    if (list_host_resp != FAILED) and\
                            (list_host_resp[0].resourcestate == 'Maintenance'):
                        break
                    sleep(30)
        if cmd_name == "deleteStorage":
            cmd_obj.forced = "true"
            ...
            if store_maint_resp != FAILED:
                ...
                for i in xrange(retries):
                    ...
                    if (store_maint_resp != FAILED) and \
...
if __name__ == "__main__":
    ...
    if options.input is None and options.remove is None:
        ...
        print "\n==== For DeleteDataCenter: Please Specify a " \
    ...
    if options.remove and os.path.isfile(options.remove) and options.input:
        ...
        '''
        remove_dc_obj = DeleteDataCenter(obj_tc_client,
