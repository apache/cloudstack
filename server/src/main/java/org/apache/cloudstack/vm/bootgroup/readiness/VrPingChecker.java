...
/**
 * PING: dispatches a dedicated {@link InstanceReadinessCheckCommand} to the VR of the VM's
 * default network. Deliberately does not reuse {@code DiagnosticsCommand}/{@code DiagnosticsType} —
 * that plumbing is a general-purpose admin tool scoped to system VMs, not user instances.
 */
@Component
public class VrPingChecker implements ReadinessChecker {
    protected static Logger LOGGER = LogManager.getLogger(VrPingChecker.class);
    ...
    public InstanceBootGroupReadinessRule.RuleType getRuleType() {
        return InstanceBootGroupReadinessRule.RuleType.Ping;
    }
    ...
    public Result check(InstanceBootGroupReadinessRule rule, Map<String, String> details, long vmId, long remainingMs) {
        ...
        LOGGER.debug("Checking ping readiness for {} due to rule {}", vm, rule);
        ...
        if (network != null && Network.GuestType.L2.equals(network.getGuestType())) {
            return logAndReturn(rule, vm, new Result(InstanceBootGroupReadinessRule.Status.Error, "The VM's default network is an L2 network; there is no VR to ping from"));
        }
        ...
        InstanceReadinessCheckCommand command = new InstanceReadinessCheckCommand(nic.getIPv4Address(),
        ...
        if (remainingMs < MIN_REMAINING_MS_TO_DISPATCH) {
            return logAndReturn(rule, vm, new Result(InstanceBootGroupReadinessRule.Status.Error,
                    "Insufficient time remaining in this attempt's budget (" + remainingMs + "ms) to dispatch a ping check"));
        }
        command.setWait(computeWaitSeconds(remainingMs));

        LOGGER.debug("Dispatching ping of {} to {} via {} with {}ms remaining budget", nic.getIPv4Address(), router, router.getHostId(), remainingMs);
        Answer answer;
        try {
            answer = agentManager.easySend(router.getHostId(), command);
        } catch (Exception e) {
            return logAndReturn(rule, vm, new Result(InstanceBootGroupReadinessRule.Status.Error, "Failed to dispatch ping via VR: " + e.getMessage()));
        }
        if (answer == null) {
        ...
        if ("0".equals(exitCode)) {
            return logAndReturn(rule, vm, new Result(InstanceBootGroupReadinessRule.Status.Ready, "ping succeeded"));
        }
        return logAndReturn(rule, vm, new Result(InstanceBootGroupReadinessRule.Status.Error, "ping failed: " + executionDetails.get(InstanceReadinessCheckAnswer.STDERR)));
    }
}
