...
(function ($, cloudStack, _l) {
    cloudStack.dialog = {
        ...
        createForm: function (args) {
            ...
            $('<span>').addClass('message').prependTo($formContainer).html(_l(formDesc));
            // Submit button
            $('<input>')
                .attr({
                type: 'submit'
            ...
            $(fields).each(function (idx, element) {
                ...
                var $name = $('<div>').addClass('name')
                    .appendTo($formItem)
                    .append($('<label>').html(_l(field.label) + ':'));
                // red asterisk
                ...
                if (field.select) {
                    ...
                    if (dependsOn) {
                        ...
                        $dependsOn.each(function () {
                            ...
                            $targetDependsOn.bind('change', function (event) {
                                ...
                                dependsOnLoaded = $.inArray(true, $dependsOn.map(function (index, item) { return $(item).find('option').length ? true : false; })) > -1;
                                if (!dependsOnLoaded) {
                                    return false;
                                }
                                var $target = $(this);
                    ...
                    if (field.multiArray) {
                        ...
                        $.each(field.multiArray, function (itemKey, itemValue) {
                            $input.append($('<div>').addClass('item')
                                .append($.merge($('<div>').addClass('name').html(_l(itemValue.label)), $('<div>').addClass('value').append($('<input>').attr({
                                name: itemKey,
                                type: 'checkbox'
                            }).appendTo($value)))));
                        });
                                ...
                                success: function (args) {
                                    if (args.data == undefined || args.data.length == 0) {
                                        ...
                                        $(args.data).each(function () {
                                            ...
                                            $input.append($('<div>')
                                                .addClass('item')
                                                .append($.merge($('<div>').addClass('name').html(_l(desc)), $('<div>').addClass('value').append($('<input>').attr({
                                                name: id,
                                                type: 'checkbox'
                                            })
                                                .data('json-obj', this)
                    ...
                    $('<input>').attr({
                        type: 'text',
                        name: field.range[1]
                    })).appendTo($('<div>').addClass('range-edit').appendTo($value));
                                    ...
                                    response: {
                                        success: function () {
                                            args.after({
                                                data: data,
                                                ref: args.ref,
                                                // For backwards compatibility; use context
                                                context: args.context,
                                                $form: $form
                                            });
                                            $('div.overlay').remove();
        ...
        confirm: function (args) {
            var $dialog = $($('<span>').addClass('message').html(_l(args.message))).dialog({
