...
class CsRoute:
    ...
    def add_table(self, devicename):
        ...
        data = "%s %s" % (tablenumber, tablename)
        filename = "/etc/iproute2/rt_tables"
        logging.info(
            "Adding route table: %s to %s if not present ", data, filename)
        CsHelper.addifmissing(filename, data)
    ...
    def add_route(self, dev, address):
        ...
        logging.info("Adding route: dev %s table: %s network: %s if not present", dev, table, address)
    ...
    def set_route(cmd, method="add"):
        ...
        found = False
        for i in CsHelper.execute("ip route show %s" % cmd):
            found = True
        if not found and method == "add":
            logging.info("Add %s", cmd)
            cmd = "ip route add %s" % cmd
        elif found and method == "delete":
            logging.info("Delete %s", cmd)
            cmd = "ip route delete %s" % cmd
        else:
            return
