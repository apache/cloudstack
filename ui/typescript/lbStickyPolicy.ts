            ...
            return function (args) {
                ...
                if ('vpc' in args.context) { //from VPC section
                    ...
                    $.ajax({
                        ...
                        //check whether the VPC has a network including Lb service
                        data: data,
                        async: false,
                ...
                var stickinessCapabilities = JSON.parse($.grep(lbService.capability, function (capability) {
                ...
                $.map($.map(stickinessCapabilities, function (c) {
                    return c.paramlist;
                }), function (p) {
               ...
               re-create sticky rule with new parameters */
                        if ($item) {
                            ...
                            cloudStack.lbStickyPolicy.actions.recreate($item.data('multi-custom-data').id, $item.data('multi-custom-data').lbRuleID, data, function () {
        ...
        actions: {
            add: function (lbRuleID, data, complete, error) {
                ...
                $(stickyParams).each(function () {
                    ...
                    var value = data[name];
                    if (!value || $.inArray(name, notParams) > -1)
                        return true;
                    if (value == 'on')
                        value = true;
            ...
            recreate: function (stickyRuleID, lbRuleID, data, complete, error) {
                var addStickyPolicy = function () {
                    cloudStack.lbStickyPolicy.actions.add(lbRuleID, data, complete, error);
                };
