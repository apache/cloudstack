    ...
    protected FirewallRule removeSshFirewallRule(final IpAddress publicIp, final long networkId) {
        ...
        for (FirewallRuleVO firewallRule : firewallRules) {
            ...
            if (rule == null) {
                ...
                if (Objects.nonNull(pfRule) && pfRule.getDestinationPortStart() == DEFAULT_SSH_PORT) {
                    rule = firewallRule;
                }
            }
        }
        if (rule != null) {
            firewallService.revokeIngressFwRule(rule.getId(), true);
            logger.debug("The SSH firewall rule {} with the id {} was revoked", rule.getName(), rule.getId());
        }
        return rule;
    }
