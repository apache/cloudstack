...
(function ($, cloudStack) {
    cloudStack.uiCustom.autoscaler = function (args) {
        ...
        return function (args) {
            ...
            var topFieldForm, $topFieldForm, bottomFieldForm, $bottomFieldForm, scaleUpPolicyTitleForm, $scaleUpPolicyTitleForm, scaleDownPolicyTitleForm, $scaleDownPolicyTitleForm, scaleUpPolicyForm, scaleDownPolicyForm;
            var renderActions = function (args) {
                ...
                $(visibleActions).map(function (index, actionID) {
                    ...
                    if (visibleActions.length == 1)
                        $action.addClass('single');
                    else if (!index)
                        $action.addClass('first');
                    else if (index == visibleActions.length - 1)
                        $action.addClass('last');
            ...
            var renderDialogContent = function (args) {
                ...
                if (data.scaleUpPolicy && $.isArray(data.scaleUpPolicy.conditions)) {
                    $autoscalerDialog.data('autoscaler-scale-up-data', data.scaleUpPolicy.conditions);
                }
                if (data.scaleUpPolicy && data.scaleUpPolicy.duration) {
                    $scaleUpPolicyTitleForm.find('input[name=scaleUpDuration]').val(data.scaleUpPolicy.duration);
                }
                ...
                if (data.scaleDownPolicy && $.isArray(data.scaleDownPolicy.conditions)) {
                    $autoscalerDialog.data('autoscaler-scale-down-data', data.scaleDownPolicy.conditions);
                }
                if (data.scaleDownPolicy && data.scaleDownPolicy.duration) {
                    $scaleDownPolicyTitleForm.find('input[name=scaleDownDuration]').val(data.scaleDownPolicy.duration);
                }
                ...
                $autoscalerDialog.append($topFields, $scaleUpPolicyTitle, $scaleUpPolicy, $scaleDownPolicyTitle, $scaleDownPolicy, $bottomFields);
                // Render dialog
