...
public class AncientDataMotionStrategy implements DataMotionStrategy {
    ...
    protected boolean needCacheStorage(DataObject srcData, DataObject destData) {
        ...
        if (s_logger.isTraceEnabled()) {
            s_logger.trace("needCacheStorage true, dest at " + destTO.getPath() + " dest role " + destStoreTO.getRole().toString() + srcTO.getPath() + " src role " +
        ...
        try {
            ...
            if (cacheData != null) {
                ...
                if (srcData.getType() == DataObjectType.VOLUME &&
                    ...
                    if (s_logger.isTraceEnabled()) s_logger.trace("Delete " + cacheType + " cache(id: " + cacheId +
                                   ", uuid: " + cacheUuid + ")");
                    ...
                    if ((answer == null || !answer.getResult()) && srcForCopy.getRefCount() < 2) {
                        ...
                        if (s_logger.isTraceEnabled()) s_logger.trace("Decrease reference count of " + cacheType +
    ...
    protected Answer cloneVolume(DataObject template, DataObject volume) {
        ...
        try {
            ...
            if (s_logger.isTraceEnabled()) s_logger.trace("Failed to send to storage pool", e);
        ...
        if (cacheStore == null) {
            ...
            try {
                ...
                if (answer == null || !answer.getResult()) {
                    if (answer != null) {
                        if (s_logger.isTraceEnabled()) s_logger.trace("copy to image store failed: " + answer.getDetails());
                    }
                ...
                if (answer == null || !answer.getResult()) {
                    if (answer != null) {
                        if (s_logger.isTraceEnabled()) s_logger.trace("copy to primary store failed: " + answer.getDetails());
                    }
                    objOnImageStore.processEvent(Event.OperationFailed);
                    imageStore.delete(objOnImageStore);
                    return answer;
    ...
    protected Answer migrateVolumeToPool(DataObject srcData, DataObject destData) {
        ...
        if (ep == null) {
            ...
            if (s_logger.isTraceEnabled()) s_logger.trace("Sending MIGRATE_COPY request to node " + ep);
            answer = ep.sendMessage(command);
            if (s_logger.isTraceEnabled()) s_logger.trace("Received MIGRATE_COPY response from node with answer: " + answer);
        }
        ...
        if (answer == null || !answer.getResult()) {
            ...
            if (s_logger.isTraceEnabled()) s_logger.trace("MIGRATE_COPY updating volume");
            ...
            if (s_logger.isTraceEnabled()) s_logger.trace("MIGRATE_COPY update volume data complete");
    ...
    public void copyAsync(DataObject srcData, DataObject destData, Host destHost, AsyncCompletionCallback<CopyCommandResult> callback) {
        ...
        String errMsg = null;
        try {
            if (s_logger.isTraceEnabled()) s_logger.trace("copyAsync inspecting src type " + srcData.getType().toString() + " copyAsync inspecting dest type " + destData.getType().toString());
            if (srcData.getType() == DataObjectType.SNAPSHOT && destData.getType() == DataObjectType.VOLUME) {
                ...
                if (s_logger.isTraceEnabled()) s_logger.trace("About to MIGRATE copy between datasources");
                if (srcData.getId() == destData.getId()) {
                    // The volume has to be migrated across storage pools.
                    if (s_logger.isTraceEnabled()) s_logger.trace("MIGRATE copy using migrateVolumeToPool STARTING");
                    answer = migrateVolumeToPool(srcData, destData);
                    if (s_logger.isTraceEnabled()) s_logger.trace("MIGRATE copy using migrateVolumeToPool DONE: " + answer.getResult());
                } else {
                    if (s_logger.isTraceEnabled()) s_logger.trace("MIGRATE copy using copyVolumeBetweenPools STARTING");
                    answer = copyVolumeBetweenPools(srcData, destData);
                    if (s_logger.isTraceEnabled()) s_logger.trace("MIGRATE copy using copyVolumeBetweenPools DONE: " + answer.getResult());
                }
    ...
    protected Answer copySnapshot(DataObject srcData, DataObject destData) {
        ...
        try {
            ...
            if (s_logger.isTraceEnabled()) s_logger.trace("copy snasphot failed: ", e);
            if (cacheData != null) {
                cacheMgr.deleteCacheObject(cacheData);
            }
            throw new CloudRuntimeException(e.toString());
