...
# host-independent gateway addresses and a MAC address derived from the routed
# id, and is therefore identical on every hypervisor - gateway addresses and
# gateway MAC alike - so an Instance needs no reconfiguration when it
# migrates, and its ARP/NDP cache stays valid.
#
...
addBr() {
    ...
    # same one: a live-migrated Instance's neighbour cache then stays valid
    # instead of stalling until the gateway entry expires and is re-learned.
    # 0e is a locally administered unicast prefix, the five id bytes make the
    # MAC unique per network, and setting it explicitly also pins it - the
