...
*/		
    ...
    synchronized void close() {
		try {
			if (sock != null && !closed) {
				...
				sock = null;
				closed = true;
				Logger.log(Logger.INFO, "RFB socket closed");
			}
			if (rec != null) {
				rec.close();
				rec = null;
  ...
  void readProxyVersion() throws Exception {
  ...
  void writeProxyString(String realhost, int realport, String sid) throws IOException {
    byte[] bs = (realhost + ":" + realport + ":" + sid + "\n").getBytes("US-ASCII");
//Logger.log(Logger.INFO, "writeProxyString " + realhost + ":" + realport );
    os.write('V');
    os.write('M');
    os.write(bs.length);
    os.write(bs);
  ...
  void readSecurityResult(String authType) throws Exception {
    ...
    case VncAuthOK:
//      Logger.log(Logger.INFO, authType + ": success");
      break;
    case VncAuthFailed:
      if (clientMinor >= 8)
        readConnFailedReason();
...
//    viewer.options.disableShareDesktop();
  ...
  void readServerInit() throws IOException {
    ...
    Logger.log(Logger.INFO, "Recv'd ServerInit w=" + framebufferWidth + 
    		" h=" + framebufferHeight + 
    		" bits=" + bitsPerPixel +
    		" depth=" + depth +
    		" name=" + desktopName);
  ...
  void readFramebufferUpdateRectHdr() throws Exception {
	...
	if (!zlibWarningShown) {
	  Logger.log(Logger.INFO, "Warning: Raw encoding will be used " +
	...
	if (updateRectEncoding == EncodingTight && !recordFromBeginning &&
	    !tightWarningShown) {
	  Logger.log(Logger.INFO, "Warning: Re-compressing Tight-encoded " +
  ...
  void writeClientCustomMessage(int type)
       throws IOException
  {
    byte[] b = new byte[2];
