...
(function ($, cloudStack, _l, _s) {
    var uiActions = {
        standard: function ($instanceRow, args, additional) {
            ...
            if (preAction) {
                ...
                preActionContext[listViewArgs.activeSection] = (multiSelect ? data.jsonObj : [data.jsonObj]);
                if (!preAction({
            ...
            var performAction = function (data, options) {
                ...
                context[listViewArgs.activeSection] = (multiSelect ? $.map($instanceRow, function (elem) { return $(elem).data('jsonObj'); }) : [$instanceRow.data('jsonObj')]);
                ...
                if (externalLinkAction) {
                    ...
                    action.custom({
                        ...
                        complete: function (args) {
                            ...
                            cloudStack.ui.notifications.add(notification, function (args) {
                                if (listViewArgs.onActionComplete) {
                                    listViewArgs.onActionComplete();
                                }
                                if ($item.is(':visible') && !isHeader) {
                                    replaceItem($item, args.data, args.actionFilter ?
                            ...
                            function (args) {
                                if (args && args.updatedData) {
                                    if ($item.is(':visible') && !isHeader) {
                                        replaceItem($item, args.updatedData, args.actionFilter);
                                    }
                                }
                                else {
                                    $item.remove();
                    ...
                    var actionArgs = {
                        ...
                        response: {
                            success: function (args) {
                                ...
                                if (args.data && (!isHeader || multiSelect)) {
                                    if (multiSelect) {
                                        $instanceRow = $.map($instanceRow, function (elem, index) {
                                            return replaceItem($(elem), $.extend($(elem).data('json-obj'), args.data[index]), $(elem).data('list-view-action-filter'))[0];
                                        });
                                    }
                                    else {
                                        $instanceRow = replaceItem($instanceRow, $.extend($instanceRow.data('json-obj'), args.data), $instanceRow.data('list-view-action-filter'));
                                    }
                                }
                                if (multiSelect) {
                                    $.each($instanceRow, function (index, elem) {
                                        ...
                                        if (options.$item)
                                            $(elem).data('list-view-new-item', true);
                                    ...
                                    if (options.$item)
                                        $instanceRow.data('list-view-new-item', true);
                                    // Disable any clicking/actions for row
                                ...
                                notification._custom = args._custom;
                                if (additional && additional.success)
                                    additional.success(args);
                                        ...
                                        if (visible) {
                                            if (args.data) {
                                                if (multiSelect) {
                                                    $newRow = [];
                                                    $.each($instanceRow, function (index, elem) {
                                                        $newRow.push(replaceItem($(elem), args.data, //$.extend($(elem).data('json-obj'), args.data[index]),
                                                        actionFilter));
                                                    });
                                                }
                                                else {
                                                    $newRow = replaceItem($instanceRow, args.data, actionFilter);
                                                }
                                            }
                                            else {
                                                ...
                                                if (multiSelect) {
                                                    $instanceRow = $.map($instanceRow, function (elem) {
                                                        replaceItem($(elem), $(elem).data('json-obj'), actionFilter)[0];
                                                    });
                                                }
                                                else {
                                                    $newRow = replaceItem($instanceRow, $instanceRow.data('json-obj'), actionFilter);
                                                }
                                            }
                                            if (needsRefresh) {
                                    ...
                                    if (!isHeader) {
                                        if (isAddAction == true && $instanceRow.data('list-view-new-item')) {
                                            ...
                                            if (!args.notification) {
                                                if (multiSelect) {
                                                    $.each($instanceRow, function (index, elem) {
                                                        replaceItem($(elem), $.extend($(elem).data('json-obj'), errorArgs.data), errorArgs.actionFilter ?
                                                            errorArgs.actionFilter :
                                                            $(elem).data('list-view-action-filter'));
                                                    });
                                                }
                                                else {
                                                    replaceItem($instanceRow, $.extend($instanceRow.data('json-obj'), errorArgs.data), errorArgs.actionFilter ?
                                                        errorArgs.actionFilter :
                                                        $instanceRow.data('list-view-action-filter'));
                                                }
                                            }
        ...
        edit: function ($instanceRow, args) {
            ...
            var showLabel = function (val, options) {
                ...
                var context = $.extend({}, listViewArgs.context);
                context[listViewArgs.activeSection] = $instanceRow.data('jsonObj');
                args.callback({
                    data: data,
                    context: context,
                    response: {
    ...
    var renderActionCol = function (actions) {
        return $.grep($.map(actions, function (value, key) {
            return key;
        }), function (elem) {
            return elem != 'add';
        }).length;
    ...
    var createHeader = function (preFilter, fields, $table, actions, options) {
        ...
        if (reorder) {
            $tr.append($('<th>').html(_l('label.order')).addClass('reorder-actions reduced-hide'));
        }
        // Actions column
        ...
        var headerActionsArray = $.grep(actionsArray, function (action) {
    ...
    var makeActionIcons = function ($td, actions, options) {
        ...
        $.each(actions, function (actionName, action) {
            ...
            if (action.textLabel) {
                $action
                    .addClass('text')
                    .prepend($('<span>').addClass('label').html(_l(action.textLabel)));
            }
    ...
    var addTableRows = function (preFilter, fields, data, $tbody, actions, options) {
        ...
        if (!(data && data.length)) {
            ...
            if (!$tbody.find('tr').length) {
                return [
                    $('<tr>').addClass('empty last').append($('<td>').html(_l('label.no.data'))).appendTo($tbody)
                ];
            }
        }
        ...
        $(data).each(function () {
            ...
            if (reorder) {
                var sort = function ($tr, action) {
                    ...
                    var context = $.extend(true, {}, $tr.closest('.list-view').data('view-args').context);
            ...
            if (actions && !options.noActionCol && renderActionCol(actions) && actionsArray.length != headerActionsArray.length) {
                ...
                makeActionIcons($('<td></td>').addClass('actions reduced-hide')
                    .appendTo($tr), actions, {
            ...
            if (subselect) {
                ...
                var $select = $('<div></div>').addClass('subselect').append($('<span>').html(_l(subselect.label))).hide();
                var $selectionArea = $tr.find('td:last').find('input');
                if (subselect.isMultiple) {
                    $select.append($('<select multiple>'), $('<span>').addClass('info').html(_l('message.listView.subselect.multi')));
                }
                else {
                ...
                $selectionArea.change(function () {
                    if ($(this).is(':checked')) {
                        ...
                        subselect.dataProvider({
                            context: $.extend(true, {}, ($listView && $listView.data('view-args') ?
            ...
            if (detailView && !$.isFunction(detailView) && !detailView.noCompact && !uiCustom) {
                ...
                function () {
                    ...
                    // Title
                    $title.append($('<span>').html(_l('label.quickview') + ': '), $('<span>').addClass('title').html(cloudStack.concat($tr.find('td:first span').html(), 30)).attr({
                    ...
                    $quickViewTooltip.hide().appendTo('#container').fadeIn(200, function () {
                        if (!$quickViewTooltip.is(':visible'))
                            return;
                        // Init detail view
    ...
    var createSectionSwitcher = function (args) {
        ...
        if (sectionPreFilter && sectionPreFilter.length == 1) {
            ...
            $switcher.find('label').html(_l('label.viewing') + ' ' + _l(sections[sectionPreFilter[0]].title));
        }
    ...
    var makeListView = function ($container, args, section) {
        args.activeSection = section ? section : (args.listView.id ? args.listView.id : args.id);
        ...
        if ($switcher && $switcher.find('option').length == 1) {
            listViewData = args.sections[$switcher.find('select').val()].listView;
        ...
        if (listViewData.actions && listViewData.actions.add) {
            ...
            if (showAdd) {
                $toolbar
                    ...
                    .append($('<span>').html(_l(listViewData.actions.add.label))));
            }
        }
        ...
        if (listViewData.actions) {
            $.each(listViewData.actions, function (actionName, action) {
                var preFilter = function (extendContext) {
                    var context = $.extend(true, {}, $listView.data('view-args').context ? $listView.data('view-args').context : cloudStack.context);
                    if (extendContext) {
                ...
                if (!action.isHeader || (action.preFilter && !preFilter()))
                    return true;
                var $action = $('<div>')
        ...
        createHeader(listViewData.preFilter, listViewData.fields, $table, listViewData.actions, {
            reorder: reorder,
        ...
        loadBody($table, listViewData.dataProvider, listViewData.preFilter, listViewData.fields, false, {
        ...
        $listView.find('select').bind('change', function (event) {
            if ($(event.target).closest('.section-select').length)
                return true;
            if ((event.type == 'click' ||
                event.type == 'mouseup') &&
        ...
        $listView.find('.advanced-search .icon').bind('click', function (event) {
            ...
            $form.find('input[type=submit]')
                ...
                .val(_l('label.search'));
            // Cancel button
            $form.append($('<div>').addClass('button cancel').html(_l('label.cancel'))
                .click(function () {
                closeAdvancedSearch();
            }));
        ...
        $listView.bind('scroll', function (event) {
            ...
            if (listView && listView.disableInfiniteScrolling)
                return false;
            if ($listView.find('tr.last, td.loading:visible').length)
                return false;
    ...
    var prependItem = function (listView, data, actionFilter, options) {
        ...
        var targetArgs = listViewArgs.activeSection ? listViewArgs.sections[listViewArgs.activeSection].listView : listViewArgs;
        var reorder = targetArgs.reorder;
        var $tr = addTableRows(targetArgs.preFilter, targetArgs.fields, data, listView.find('table tbody'), targetArgs.actions, {
            prepend: true,
    ...
    var replaceItem = function ($row, data, actionFilter, after) {
        ...
        $newRow = addTableRows(targetArgs.preFilter, targetArgs.fields, data, $listView.find('table tbody'), targetArgs.actions, {
    ...
    $.fn.listView = function (args, options) {
        ...
        if (args == 'prependItem') {
            ...
            loadBody(this.find('table:last'), listViewArgs.dataProvider, listViewArgs.preFilter, listViewArgs.fields, false, null, listViewArgs.actions, {
                clearEndTable: true,
