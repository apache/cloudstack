...
public abstract class NioConnection implements Callable<Boolean> {
    private static final Logger LOG = LogFactory.getLogger(NioConnection.class);;
    ...
    public void start() throws NioConnectionException {
        ...
        try {
            ...
            LOG.warn("Unable to connect to remote: is there a server running on port " + _port);
            return;
        } catch (final IOException e) {
            LOG.error("Unable to initialize the threads.", e);
            throw new NioConnectionException(e.getMessage(), e);
        } catch (final Exception e) {
            LOG.error("Unable to initialize the threads due to unknown exception.", e);
            throw new NioConnectionException(e.getMessage(), e);
        }
        _isStartup = true;
        ...
        while (_isRunning) {
            try {
                ...
                if (LOG.isTraceEnabled()) {
                    LOG.trace("Keys Processing: " + readyKeys.size());
                }
                ...
                while (i.hasNext()) {
                    ...
                    if (!sk.isValid()) {
                        if (LOG.isTraceEnabled()) {
                            LOG.trace("Selection Key is invalid: " + sk.toString());
                        }
                ...
                LOG.trace("Keys Done Processing.");
                ...
                LOG.error("Agent will die due to this IOException!", e);
                throw new NioConnectionException(e.getMessage(), e);
            }
        }
        _isStartup = false;
    ...
    protected void accept(final SelectionKey key) throws IOException {
        ...
        if (LOG.isTraceEnabled()) {
            LOG.trace("Connection accepted for " + socket);
        }
            ...
            _sslHandshakeExecutor.submit(new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        if (LOG.isTraceEnabled()) {
                            LOG.trace("Connection closed due to failure: " + e.getMessage());
                        }
    ...
    protected void terminate(final SelectionKey key) {
        ...
        if (link != null) {
            ...
            try {
                ...
                LOG.warn("Exception occurred when submitting the task", e);
            }
        }
    }
    ...
    protected void read(final SelectionKey key) throws IOException {
        ...
        try {
            ...
            if (LOG.isTraceEnabled()) {
                LOG.trace("Reading from: " + socketChannel.socket().toString());
            }
            final byte[] data = link.read(socketChannel);
            if (data == null) {
                if (LOG.isTraceEnabled()) {
                    LOG.trace("Packet is incomplete.  Waiting for more.");
                }
                return;
            }
    ...
    protected void logTrace(final Exception e, final SelectionKey key, final int loc) {
        if (LOG.isTraceEnabled()) {
            ...
            LOG.trace("Location " + loc + ": Socket " + socket + " closed on read.  Probably -1 returned.");
        }
    }
    ...
    protected void logDebug(final Exception e, final SelectionKey key, final int loc) {
        if (LOG.isDebugEnabled()) {
            ...
            LOG.debug("Location " + loc + ": Socket " + socket + " closed on read.  Probably -1 returned: " + e.getMessage());
        }
    }
    ...
    protected void processTodos() {
        ...
        if (LOG.isTraceEnabled()) {
            LOG.trace("Todos Processing: " + todos.size());
        }
        SelectionKey key;
        for (final ChangeRequest todo : todos) {
            ...
            case ChangeRequest.CHANGEOPS:
                try {
                    ...
                    LOG.debug("key has been cancelled");
                }
                break;
            case ChangeRequest.REGISTER:
                try {
                    ...
                    try {
                        ...
                        LOG.info("[ignored] socket channel");
                    } finally {
                        final Link link = (Link)todo.att;
                        link.terminated();
                    }
            ...
            case ChangeRequest.CLOSE:
                if (LOG.isTraceEnabled()) {
                    LOG.trace("Trying to close " + todo.key);
                }
                key = (SelectionKey)todo.key;
                closeConnection(key);
                if (key != null) {
        ...
        LOG.trace("Todos Done processing");
    }
    ...
    protected void write(final SelectionKey key) throws IOException {
        ...
        try {
            if (LOG.isTraceEnabled()) {
                LOG.trace("Writing to " + link.getSocketAddress().toString());
            }
    ...
    protected void closeConnection(final SelectionKey key) {
        if (key != null) {
            ...
            try {
                if (channel != null) {
                    if (LOG.isDebugEnabled()) {
                        LOG.debug("Closing socket " + channel.socket());
                    }
                    channel.close();
                }
            } catch (final IOException ignore) {
                LOG.info("[ignored] channel");
            }
        }
    }
