...
(function (cloudStack, $) {
    var pageElems = cloudStack.uiCustom.projectsTabs = {
        ...
        dashboard: function () {
            ...
            if (isAdmin() || isDomainAdmin() || (cloudStack.context.projects &&
            ...
            return $('<div>').addClass('project-dashboard')
                .append($.merge($toolbar, $tabs.tabs()));
        },
        /**
                ...
                cloudStack.projects.add({
                    ...
                    response: {
                        success: function (args) {
                            ...
                            $projectSwitcher.find('select').append($('<option>').val(project.id).html(project.name));
                            $loading.remove();
                            // Confirmation
                            $form.replaceWith(function () {
                                ...
                                $('<span>').addClass('value').html(_s(args.data.name)));
                                $confirm.append($projectDesc).find('input').replaceWith(// Display text
                                $('<span>').addClass('value').html(_s(args.data.displayText)));
                                ...
                                $addAccountButton.click(function () {
                                    ...
                                    $confirm.replaceWith(function () {
                                        ...
                                        $newProject.find('.title').html(cloudStack.projects.requireInvitation() ?
                                        ...
                                        $nextButton.appendTo($userManagement).click(function () {
                                            ...
                                            $userManagement.replaceWith(function () {
                                                ...
                                                var $ul = $('<ul>').appendTo($tabs)
                                                    .append(
                                                // Users tab
                                                $('<li>').addClass('first').append($('<a>').attr({
                                                    href: '#new-project-review-tabs-users'
                                                }).html(cloudStack.projects.requireInvitation() ?
                                                    _l('label.invitations') : _l('label.accounts'))));
                                                if (isAdmin() || isDomainAdmin()) {
                                                    $ul.append(
                                                    // Resources tab
                                                    $('<li>').addClass('last').append($('<a>').attr({
                                ...
                                $buttons.appendTo($confirm).append($.merge($addAccountButton, $laterButton));
                                return $confirm;
                            });
                        },
