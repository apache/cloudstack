...
@APICommand(name = "migrateBucketCredential", description = "Gives an existing bucket that still uses its account's credential a dedicated credential with its own rotatable keys. The account's credential keeps working on the bucket until it is revoked by the operator. Requires the account to have been migrated on the object store with migrateObjectStoreAccount where the provider needs it.",
        responseObject = BucketResponse.class, entityType = {Bucket.class},
        requestHasSensitiveInfo = false, responseHasSensitiveInfo = true, since = "24.0.0",
        authorized = {RoleType.Admin, RoleType.ResourceAdmin, RoleType.DomainAdmin, RoleType.User})
public class MigrateBucketCredentialCmd extends BaseCmd {
    ...
    public void execute() {
        ...
        Bucket bucket;
        try {
            bucket = _bucketService.migrateBucketCredential(this, CallContext.current().getCallingAccount());
        } catch (InvalidParameterValueException e) {
            throw new ServerApiException(ApiErrorCode.PARAM_ERROR, e.getMessage());
        } catch (Exception e) {
            throw new ServerApiException(ApiErrorCode.INTERNAL_ERROR, "Error while migrating bucket credential. " + e.getMessage());
        }
