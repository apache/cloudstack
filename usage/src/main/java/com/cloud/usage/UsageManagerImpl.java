...
public class UsageManagerImpl extends ManagerBase implements UsageManager, Runnable {
    ...
    public void parse(UsageJobVO job, long startDateMillis, long endDateMillis) {
        ...
        try {
            ...
            try {
                ...
                if (s_logger.isDebugEnabled()) {
                    s_logger.debug("Created Instance disk stats helper entries for " + numAcctsProcessed + " accts");
                }
                ...
                while (!currentEndDate.after(endDate) || (currentEndDate.getTime() - endDate.getTime() < 60000)) {
                    ...
                    if (s_logger.isDebugEnabled()) {
                        s_logger.debug("Processed Instance/Network Usage for " + numAcctsProcessed + " ACTIVE accts");
                    }
                    numAcctsProcessed = 0;
    ...
    private boolean parseHelperTables(AccountVO account, Date currentStartDate, Date currentEndDate) {
        ...
        if (s_logger.isDebugEnabled()) {
            if (!parsed) {
                s_logger.debug("Instances usage successfully parsed? " + parsed + " (for Account: " + account.getAccountName() + ", id: " + account.getId() + ")");
            }
        }
        ...
        if (s_logger.isDebugEnabled()) {
            if (!parsed) {
                s_logger.debug("Instance disk usage successfully parsed? " + parsed + " (for Account: " + account.getAccountName() + ", id: " + account.getId() + ")");
            }
        }
        ...
        if (s_logger.isDebugEnabled()) {
            if (!parsed) {
                s_logger.debug("Instance Snapshot usage successfully parsed? " + parsed + " (for Account: " + account.getAccountName() + ", id: " + account.getId() + ")");
            }
        }
        parsed = VMSnapshotOnPrimaryParser.parse(account, currentStartDate, currentEndDate);
        if (s_logger.isDebugEnabled()) {
            if (!parsed) {
                s_logger.debug("Instance Snapshot on primary usage successfully parsed? " + parsed + " (for Account: " + account.getAccountName() + ", id: " + account.getId() + ")");
            }
        }
        parsed = BackupUsageParser.parse(account, currentStartDate, currentEndDate);
        if (s_logger.isDebugEnabled()) {
            if (!parsed) {
                s_logger.debug("Instance Backup usage successfully parsed? " + parsed + " (for Account: " + account.getAccountName() + ", id: " + account.getId() + ")");
            }
        }
        return parsed;
    }
        ...
        if (EventTypes.EVENT_VM_START.equals(event.getType())) {
            ...
            try {
                ...
                if (usageInstances != null) {
                    if (usageInstances.size() > 0) {
                        s_logger.error("Found entries for a Instance running with id: " + vmId + ", which are not stopped. Ending them all...");
                ...
                if (usageInstances == null || (usageInstances.size() == 0)) {
                    s_logger.error("Cannot find allocated Instance entry for an Instance running with id: " + vmId);
                ...
                s_logger.error("Error saving Instance usage for Instance: " + vmId, ex);
            }
            ...
            if (usageInstances != null) {
                if (usageInstances.size() > 1) {
                    s_logger.warn("Found multiple entries for a Instance running with id: " + vmId + ", ending them all...");
                }
                for (UsageVMInstanceVO usageInstance : usageInstances) {
                    usageInstance.setEndDate(event.getCreateDate());
            ...
            try {
                ...
                s_logger.error("Error saving usage for Instance: " + vmId, ex);
            }
            ...
            if (usageInstances != null) {
                if (usageInstances.size() > 1) {
                    s_logger.warn("Found multiple entries for an Instance allocated with id: " + vmId + ", destroying them all...");
                }
                for (UsageVMInstanceVO usageInstance : usageInstances) {
                    usageInstance.setEndDate(event.getCreateDate());
                    _usageInstanceDao.update(usageInstance);
            ...
            if (usageInstances != null) {
                if (usageInstances.size() > 1) {
                    s_logger.warn("Found multiple entries for an Instance allocated with id: " + vmId + ", updating end_date for all of them...");
                }
                for (UsageVMInstanceVO usageInstance : usageInstances) {
                    usageInstance.setEndDate(event.getCreateDate());
                    _usageInstanceDao.update(usageInstance);
    ...
    private void createVmDiskHelperEntry(VmDiskStatisticsVO vmDiskStat, UsageVmDiskVO usageVmDiskStat, long timestamp) {
        ...
        if (ioRead < 0) {
            s_logger.warn("Calculated negative value for IO read: " + toHumanReadableSize(ioRead) + ", Instance disk stats say: " + toHumanReadableSize(vmDiskStat.getAggIORead()) + ", previous vm disk usage was: " +
    ...
    private void createNetworkOfferingEvent(UsageEventVO event) {
        ...
        if (EventTypes.EVENT_NETWORK_OFFERING_CREATE.equals(event.getType()) || EventTypes.EVENT_NETWORK_OFFERING_ASSIGN.equals(event.getType())) {
            if (s_logger.isDebugEnabled()) {
                s_logger.debug("Creating networking offering: " + networkOfferingId + " for Instance: " + vmId + " for Account: " + event.getAccountId());
            }
            zoneId = event.getZoneId();
            Account acct = _accountDao.findByIdIncludingRemoved(event.getAccountId());
    ...
    protected void createUsageVMSnapshot(UsageEventVO event) {
        ...
        s_logger.debug(String.format("Creating usage Instance Snapshot for Instance id [%s] assigned to account [%s] domain [%s], zone [%s], and created at [%s]", vmId, accountId, domainId, zoneId,
                event.getCreateDate()));
    ...
    protected void deleteUsageVMSnapshot(UsageEventVO event) {
        ...
        if (CollectionUtils.isEmpty(usageVMSnapshots)){
            s_logger.warn(String.format("No usage entry for Instance Snapshot for Instance id [%s] assigned to account [%s] domain [%s] and zone [%s] was found.",
                    vmId, accountId, domainId, zoneId));
        }
        if (usageVMSnapshots.size() > 1) {
            s_logger.warn(String.format("More than one usage entry for Instance Snapshot for Instance id [%s] assigned to account [%s] domain [%s] and zone [%s]; marking them all as deleted.", vmId,
                    accountId, domainId, zoneId));
        }
        for (UsageVMSnapshotVO vmSnapshots : usageVMSnapshots) {
            s_logger.debug(String.format("Deleting Instance Snapshot for Instance id [%s] assigned to account [%s] domain [%s] and zone [%s] that was created at [%s].", vmSnapshots.getVmId(),
    ...
    private void createVmSnapshotOnPrimaryEvent(UsageEventVO event) {
        ...
        if (EventTypes.EVENT_VM_SNAPSHOT_ON_PRIMARY.equals(event.getType())) {
            ...
            if (vmsnaps.size() > 1) {
                s_logger.warn("More that one usage entry for Instance Snapshot: " + name + " for Instance id:" + vmId + " assigned to account: " + event.getAccountId()
                        + "; marking them all as deleted...");
            }
            for (UsageSnapshotOnPrimaryVO vmsnap : vmsnaps) {
                if (s_logger.isDebugEnabled()) {
                    s_logger.debug("Deleting Instance Snapshot name: " + vmsnap.getName() + " from account: " + vmsnap.getAccountId());
                }
