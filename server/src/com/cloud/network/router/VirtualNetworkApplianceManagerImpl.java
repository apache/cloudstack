...
Configurable, StateListener<VirtualMachine.State, VirtualMachine.Event, VirtualMachine> {
    ...
    @Inject
    private DataCenterDao dcDao = null;
    @Inject
    VlanDao vlanDao = null;
    @Inject
    private FirewallRulesDao rulesDao = null;
    @Inject
    private LoadBalancerDao loadBalancerDao = null;
    @Inject
    IPAddressDao ipAddressDao = null;
    @Inject
    DomainRouterDao routerDao = null;
    @Inject
    private UserDao userDao = null;
    @Inject
    UserStatisticsDao userStatsDao = null;
    @Inject
    private HostDao hostDao = null;
    @Inject
    private ConfigurationDao configDao;
    @Inject
    private HostPodDao podDao = null;
    @Inject
    private UserStatsLogDao userStatsLogDao = null;
    @Inject
    ...
    @Inject
    private ServiceOfferingDao serviceOfferingDao = null;
    @Inject
    private NetworkOfferingDao networkOfferingDao = null;
    @Inject
    ...
    @Inject
    private PortForwardingRulesDao pfRulesDao;
    @Inject
    RemoteAccessVpnDao vpnDao;
    @Inject
    NicDao nicDao;
    @Inject
    private NicIpAliasDao nicIpAliasDao;
    @Inject
    private UserVmDetailsDao vmDetailsDao;
    @Inject
    VirtualRouterProviderDao vrProviderDao;
    @Inject
    private ManagementServerHostDao msHostDao;
    @Inject
    private Site2SiteCustomerGatewayDao s2sCustomerGatewayDao;
    @Inject
    private Site2SiteVpnConnectionDao s2sVpnConnectionDao;
    @Inject
    private Site2SiteVpnManager s2sVpnMgr;
    @Inject
    private NetworkService networkSvc;
    @Inject
    private MonitoringServiceDao monitorServiceDao;
    @Inject
    ...
    @Inject
    protected NetworkTopologyContext networkTopologyContext;
    ...
    private int routerRamSize;
    private int routerCpuMHz;
    private String mgmtCidr;

    private int routerStatsInterval = 300;
    private int routerCheckInterval = 30;
    private int rvrStatusUpdatePoolSize = 10;
    private String dnsBasicZoneUpdates = "all";
    private final Set<String> guestOSNeedGatewayOnNonDefaultNetwork = new HashSet<String>();

    private boolean disableRpFilter = false;
    private int routerExtraPublicNics = 2;
    private int usageAggregationRange = 1440;
    private String usageTimeZone = "GMT";
    private final long mgmtSrvrId = MacAddress.getMacAddress().toLong();
    /**
     * 5 seconds
     */
    private static final int ACQUIRE_GLOBAL_LOCK_TIMEOUT_FOR_COOPERATION = 5;
    private boolean dailyOrHourly = false;

    private ScheduledExecutorService executor;
    private ScheduledExecutorService checkExecutor;
    private ScheduledExecutorService networkStatsUpdateExecutor;
    private ExecutorService rvrStatusUpdateExecutor;

    private BlockingQueue<Long> vrUpdateQueue = null;
    ...
    public VirtualRouter upgradeRouter(final UpgradeRouterCmd cmd) {
        ...
        final ServiceOffering newServiceOffering = entityMgr.findById(ServiceOffering.class, serviceOfferingId);
        ...
        final ServiceOfferingVO currentServiceOffering = serviceOfferingDao.findById(router.getServiceOfferingId());
    ...
    public VirtualRouter stopRouter(final long routerId, final boolean forced) throws ResourceUnavailableException, ConcurrentOperationException {
        ...
        accountMgr.checkAccess(account, null, true, router);

        final UserVO user = userDao.findById(CallContext.current().getCallingUserId());
    ...
    public boolean configure(final String name, final Map<String, Object> params) throws ConfigurationException {
        ...
        executor = Executors.newScheduledThreadPool(1, new NamedThreadFactory("RouterMonitor"));
        checkExecutor = Executors.newScheduledThreadPool(1, new NamedThreadFactory("RouterStatusMonitor"));
        networkStatsUpdateExecutor = Executors.newScheduledThreadPool(1, new NamedThreadFactory("NetworkStatsUpdater"));

        VirtualMachine.State.getStateMachine().registerListener(this);

        final Map<String, String> configs = configDao.getConfiguration("AgentManager", params);

        routerRamSize = NumbersUtil.parseInt(configs.get("router.ram.size"), DEFAULT_ROUTER_VM_RAMSIZE);
        routerCpuMHz = NumbersUtil.parseInt(configs.get("router.cpu.mhz"), DEFAULT_ROUTER_CPU_MHZ);

        routerExtraPublicNics = NumbersUtil.parseInt(configDao.getValue(Config.RouterExtraPublicNics.key()), 2);
        ...
        if (guestOSString != null) {
            ...
            for (final String os : guestOSList) {
                guestOSNeedGatewayOnNonDefaultNetwork.add(os);
            }
        }
        ...
        routerStatsInterval = NumbersUtil.parseInt(value, 300);

        value = configs.get("router.check.interval");
        routerCheckInterval = NumbersUtil.parseInt(value, 30);

        value = configs.get("router.check.poolsize");
        rvrStatusUpdatePoolSize = NumbersUtil.parseInt(value, 10);
         ...
         */
        vrUpdateQueue = new LinkedBlockingQueue<Long>(rvrStatusUpdatePoolSize * 1000);

        rvrStatusUpdateExecutor = Executors.newFixedThreadPool(rvrStatusUpdatePoolSize, new NamedThreadFactory("RedundantRouterStatusMonitor"));
        ...
        dnsBasicZoneUpdates = String.valueOf(configDao.getValue(Config.DnsBasicZoneUpdates.key()));

        s_logger.info("Router configurations: " + "ramsize=" + routerRamSize);

        agentMgr.registerForHostEvents(new SshKeysDistriMonitor(agentMgr, hostDao, configDao), true, false, false);

        final List<ServiceOfferingVO> offerings = serviceOfferingDao.createSystemServiceOfferings("System Offering For Software Router",
                ServiceOffering.routerDefaultOffUniqueName,
                1, routerRamSize, routerCpuMHz, null,
        ...
        NetworkHelperImpl.setSystemAccount(accountMgr.getSystemAccount());

        final String aggregationRange = configs.get("usage.stats.job.aggregation.range");
        usageAggregationRange = NumbersUtil.parseInt(aggregationRange, 1440);
        usageTimeZone = configs.get("usage.aggregation.timezone");
        if (usageTimeZone == null) {
            usageTimeZone = "GMT";
        }
    ...
    public boolean start() {
        if (routerStatsInterval > 0) {
            executor.scheduleAtFixedRate(new NetworkUsageTask(), routerStatsInterval, routerStatsInterval, TimeUnit.SECONDS);
        } else {
            s_logger.debug("router.stats.interval - " + routerStatsInterval + " so not scheduling the router stats thread");
        }
        ...
        final TimeZone usageTimezone = TimeZone.getTimeZone(usageTimeZone);
        ...
        if (usageAggregationRange == DAILY_TIME) {
            ...
            dailyOrHourly = true;
        } else if (usageAggregationRange == HOURLY_TIME) {
            cal.set(Calendar.MINUTE, 0);
            cal.set(Calendar.SECOND, 0);
            cal.set(Calendar.MILLISECOND, 0);
        ...
        if (usageAggregationRange < UsageUtils.USAGE_AGGREGATION_RANGE_MIN) {
            s_logger.warn("Usage stats job aggregation range is to small, using the minimum value of " + UsageUtils.USAGE_AGGREGATION_RANGE_MIN);
            usageAggregationRange = UsageUtils.USAGE_AGGREGATION_RANGE_MIN;
        }
        ...
        networkStatsUpdateExecutor.scheduleAtFixedRate(new NetworkStatsUpdateTask(), initialDelay, usageAggregationRange * 60 * 1000,
                TimeUnit.MILLISECONDS);

        if (routerCheckInterval > 0) {
            checkExecutor.scheduleAtFixedRate(new CheckRouterTask(), routerCheckInterval, routerCheckInterval, TimeUnit.SECONDS);
            for (int i = 0; i < rvrStatusUpdatePoolSize; i++) {
                rvrStatusUpdateExecutor.execute(new RvRStatusUpdateTask());
            }
        } else {
            s_logger.debug("router.check.interval - " + routerCheckInterval + " so not scheduling the redundant router checking thread");
        }

        final int routerAlertsCheckInterval = RouterAlertsCheckInterval.value();
        if (routerAlertsCheckInterval > 0) {
            checkExecutor.scheduleAtFixedRate(new CheckRouterAlertsTask(), routerAlertsCheckInterval, routerAlertsCheckInterval, TimeUnit.SECONDS);
        } else {
    ...
    protected class NetworkUsageTask extends ManagedContextRunnable {
        ...
        protected void runInContext() {
            try {
                final List<DomainRouterVO> routers = routerDao.listByStateAndNetworkType(VirtualMachine.State.Running, GuestType.Isolated, mgmtSrvrId);
                ...
                for (final DomainRouterVO router : routers) {
                    ...
                    if (privateIP != null) {
                        ...
                        final List<? extends Nic> routerNics = nicDao.listByVmId(router.getId());
    ...
    protected void updateSite2SiteVpnConnectionState(final List<DomainRouterVO> routers) {
        for (final DomainRouterVO router : routers) {
            final List<Site2SiteVpnConnectionVO> conns = s2sVpnMgr.getConnectionsForRouter(router);
            ...
            if (router.getState() != VirtualMachine.State.Running) {
                for (final Site2SiteVpnConnectionVO conn : conns) {
                    if (conn.getState() != Site2SiteVpnConnection.State.Error) {
                        ...
                        s2sVpnConnectionDao.persist(conn);
                    }
                }
                continue;
            }
            ...
            for (final Site2SiteVpnConnectionVO conn : conns) {
                ...
                final Site2SiteCustomerGateway gw = s2sCustomerGatewayDao.findById(conn.getCustomerGatewayId());
            ...
            if (host == null || host.getState() != Status.Up) {
                ...
                for (final Site2SiteVpnConnectionVO conn : conns) {
                    final Site2SiteVpnConnectionVO lock = s2sVpnConnectionDao.acquireInLockTable(conn.getId());
                    if (lock == null) {
                    ...
                    try {
                        ...
                        if (oldState != conn.getState()) {
                            ...
                            alertMgr.sendAlert(AlertManager.AlertType.ALERT_TYPE_DOMAIN_ROUTER, router.getDataCenterId(), router.getPodIdToDeployIn(), title, context);
                        }
                    } finally {
                        s2sVpnConnectionDao.releaseFromLockTable(lock.getId());
                    }
                }
            }
        }
    ...
    protected void recoverRedundantNetwork(final DomainRouterVO masterRouter, final DomainRouterVO backupRouter) {
        if (masterRouter.getState() == VirtualMachine.State.Running && backupRouter.getState() == VirtualMachine.State.Running) {
            final HostVO masterHost = hostDao.findById(masterRouter.getHostId());
            final HostVO backupHost = hostDao.findById(backupRouter.getHostId());
            if (masterHost.getState() == Status.Up && backupHost.getState() == Status.Up) {
                ...
                alertMgr.sendAlert(AlertManager.AlertType.ALERT_TYPE_DOMAIN_ROUTER, backupRouter.getDataCenterId(), backupRouter.getPodIdToDeployIn(), title, title);
                try {
    ...
    protected class RvRStatusUpdateTask extends ManagedContextRunnable {
        ...
        private void checkSanity(final List<DomainRouterVO> routers) {
            ...
            for (final DomainRouterVO router : routers) {
                ...
                for (final Long routerGuestNtwkId : routerGuestNtwkIds) {
                    ...
                    if (vpcId != null) {
                        checkingRouters = routerDao.listByVpcId(vpcId);
                    } else {
                        checkingRouters = routerDao.listByNetworkAndRole(routerGuestNtwkId, Role.VIRTUAL_ROUTER);
                    }
                ...
                for (final Long routerGuestNtwkId : routerGuestNtwkIds) {
                    if (router.getRedundantState() == RedundantState.MASTER) {
                        if (networkRouterMaps.containsKey(routerGuestNtwkId)) {
                            ...
                            alertMgr.sendAlert(AlertManager.AlertType.ALERT_TYPE_DOMAIN_ROUTER, router.getDataCenterId(), router.getPodIdToDeployIn(), title, context);
                            alertMgr.sendAlert(AlertManager.AlertType.ALERT_TYPE_DOMAIN_ROUTER, dupRouter.getDataCenterId(), dupRouter.getPodIdToDeployIn(), title, context);
        ...
        protected void runInContext() {
            while (true) {
                try {
                    final Long networkId = vrUpdateQueue.take(); // This is a blocking call so this thread won't run all the time if no work item in queue.
                    ...
                    if (vpcId != null) {
                        routers = routerDao.listByVpcId(vpcId);
                    } else {
                        routers = routerDao.listByNetworkAndRole(networkId, Role.VIRTUAL_ROUTER);
                    }
    ...
    protected class CheckRouterTask extends ManagedContextRunnable {
        ...
        protected void pushToUpdateQueue(final List<NetworkVO> networks) throws InterruptedException {
            for (final NetworkVO network : networks) {
                if (!vrUpdateQueue.offer(network.getId(), 500, TimeUnit.MILLISECONDS)) {
    ...
    public boolean finalizeVirtualMachineProfile(final VirtualMachineProfile profile, final DeployDestination dest, final ReservationContext context) {
        ...
        final DataCenterVO dc = dcDao.findById(dest.getDataCenter().getId());
        dcDao.loadDetails(dc);

        // 1) Set router details
        final DomainRouterVO router = routerDao.findById(profile.getVirtualMachine().getId());
        final Map<String, String> details = vmDetailsDao.listDetailsKeyPairs(router.getId());
        ...
        if (Boolean.valueOf(configDao.getValue("system.vm.random.password"))) {
            buf.append(" vmpassword=").append(configDao.getValue("system.vm.password"));
        }
        ...
        for (final NicProfile nic : profile.getNics()) {
            ...
            if (nic.getTrafficType() == TrafficType.Management) {
                ...
                if (dest.getHost().getHypervisorType() == HypervisorType.VMware || dest.getHost().getHypervisorType() == HypervisorType.Hyperv) {
                    ...
                    mgmtCidr = configDao.getValue(Config.ManagementNetwork.key());
                    if (NetUtils.isValidCIDR(mgmtCidr)) {
                ...
                dnsProvided = networkModel.isProviderSupportServiceInNetwork(nic.getNetworkId(), Service.Dns, Provider.VirtualRouter);
                dhcpProvided = networkModel.isProviderSupportServiceInNetwork(nic.getNetworkId(), Service.Dhcp, Provider.VirtualRouter);
        ...
        final String rpValue = configDao.getValue(Config.NetworkRouterRpFilter.key());
        if (rpValue != null && rpValue.equalsIgnoreCase("true")) {
            disableRpFilter = true;
        } else {
        ...
        if (profile.getHypervisorType() == HypervisorType.VMware || profile.getHypervisorType() == HypervisorType.Hyperv) {
            buf.append(" extra_pubnics=" + routerExtraPublicNics);
        }
        ...
        if (Boolean.valueOf(configDao.getValue(Config.BaremetalProvisionDoneNotificationEnabled.key()))) {
            ...
            if (user == null) {
                ...
                buf.append(" port=").append(configDao.getValue(Config.BaremetalProvisionDoneNotificationPort.key()));
            }
        }
    ...
    protected StringBuilder createGuestBootLoadArgs(final NicProfile guestNic, final String defaultDns1, final String defaultDns2, final DomainRouterVO router) {
        ...
        final NetworkVO guestNetwork = networkDao.findById(guestNetworkId);
        String dhcpRange = null;
        final DataCenterVO dc = dcDao.findById(guestNetwork.getDataCenterId());
        ...
        if (isRedundant) {
            ...
            final Network net = networkModel.getNetwork(guestNic.getNetworkId());
            ...
            final int advertInt = NumbersUtil.parseInt(configDao.getValue(Config.RedundantRouterVrrpInterval.key()), 1);
    ...
    protected StringBuilder createRedundantRouterArgs(final NicProfile nic, final DomainRouterVO router) {
        ...
        if (isRedundant) {
            ...
            if (vpcId != null) {
                ...
                routers = routerDao.listByNetworkAndRole(nic.getNetworkId(), Role.VIRTUAL_ROUTER);
            }
    ...
    public boolean finalizeCommandsOnStart(final Commands cmds, final VirtualMachineProfile profile) {
        ...
        for (final Long guestNetworkId : routerGuestNtwkIds) {
            final AggregationControlCommand startCmd = new AggregationControlCommand(Action.Start, router.getInstanceName(), controlNic.getIPv4Address(), routerControlHelper.getRouterIpInNetwork(
            ...
            if (reprogramGuestNtwks) {
                ...
                final NetworkOffering offering = networkOfferingDao.findById(networkDao.findById(guestNetworkId).getNetworkOfferingId());
                // service monitoring is currently not added in RVR
                if (!offering.getRedundantRouter()) {
                    final String serviceMonitringSet = configDao.getValue(Config.EnableServiceMonitoring.key());
            ...
            final AggregationControlCommand finishCmd = new AggregationControlCommand(Action.Finish, router.getInstanceName(), controlNic.getIPv4Address(), routerControlHelper.getRouterIpInNetwork(
                    guestNetworkId, router.getId()));
    ...
    private void finalizeMonitorServiceOnStrat(final Commands cmds, final VirtualMachineProfile profile, final DomainRouterVO router, final Provider provider,
        ...
        if (networkModel.isProviderSupportServiceInNetwork(network.getId(), Service.Dhcp, Provider.VirtualRouter)
                || networkModel.isProviderSupportServiceInNetwork(network.getId(), Service.Dns, Provider.VirtualRouter)) {
            final MonitoringServiceVO dhcpService = monitorServiceDao.getServiceByName(MonitoringService.Service.Dhcp.toString());
            if (dhcpService != null) {
                services.add(dhcpService);
            }
        }

        if (networkModel.isProviderSupportServiceInNetwork(network.getId(), Service.Lb, Provider.VirtualRouter)) {
            final MonitoringServiceVO lbService = monitorServiceDao.getServiceByName(MonitoringService.Service.LoadBalancing.toString());
            if (lbService != null) {
                services.add(lbService);
            }
        }
        final List<MonitoringServiceVO> defaultServices = monitorServiceDao.listDefaultServices(true);
        ...
        command.setAccessDetail(NetworkElementCommand.ROUTER_GUEST_IP, routerControlHelper.getRouterIpInNetwork(networkId, router.getId()));
        command.setAccessDetail(NetworkElementCommand.ROUTER_NAME, router.getInstanceName());
    ...
    protected void finalizeSshAndVersionAndNetworkUsageOnStart(final Commands cmds, final VirtualMachineProfile profile, final DomainRouterVO router, final NicProfile controlNic) {
        final DomainRouterVO vr = routerDao.findById(profile.getId());
    ...
    protected void finalizeUserDataAndDhcpOnStart(final Commands cmds, final DomainRouterVO router, final Provider provider, final Long guestNetworkId) {
        if (networkModel.isProviderSupportServiceInNetwork(guestNetworkId, Service.Dhcp, provider)) {
            // Resend dhcp
            s_logger.debug("Reapplying dhcp entries as a part of domR " + router + " start...");
            commandSetupHelper.createDhcpEntryCommandsForVMs(router, cmds, guestNetworkId);
        }

        if (networkModel.isProviderSupportServiceInNetwork(guestNetworkId, Service.UserData, provider)) {
            // Resend user data
            s_logger.debug("Reapplying vm data (userData and metaData) entries as a part of domR " + router + " start...");
            commandSetupHelper.createVmDataCommandForVMs(router, cmds, guestNetworkId);
        }
    }
    ...
    protected void finalizeNetworkRulesForNetwork(final Commands cmds, final DomainRouterVO router, final Provider provider, final Long guestNetworkId) {
        ...
        if (networkModel.isProviderSupportServiceInNetwork(guestNetworkId, Service.Firewall, provider)) {
            firewallRulesEgress.addAll(rulesDao.listByNetworkPurposeTrafficType(guestNetworkId, Purpose.Firewall, FirewallRule.TrafficType.Egress));
        ...
        if (!firewallRulesEgress.isEmpty()) {
            commandSetupHelper.createFirewallRulesCommands(firewallRulesEgress, router, cmds, guestNetworkId);
        }
        ...
        if (publicIps != null && !publicIps.isEmpty()) {
            ...
            for (final PublicIpAddress ip : publicIps) {
                if (networkModel.isProviderSupportServiceInNetwork(guestNetworkId, Service.PortForwarding, provider)) {
                    pfRules.addAll(pfRulesDao.listForApplication(ip.getId()));
                }
                if (networkModel.isProviderSupportServiceInNetwork(guestNetworkId, Service.StaticNat, provider)) {
                    staticNatFirewallRules.addAll(rulesDao.listByIpAndPurpose(ip.getId(), Purpose.StaticNat));
                }
                if (networkModel.isProviderSupportServiceInNetwork(guestNetworkId, Service.Firewall, provider)) {
                    firewallRulesIngress.addAll(rulesDao.listByIpAndPurpose(ip.getId(), Purpose.Firewall));
                }

                if (networkModel.isProviderSupportServiceInNetwork(guestNetworkId, Service.Vpn, provider)) {
                    final RemoteAccessVpn vpn = vpnDao.findByPublicIpAddress(ip.getId());
                    if (vpn != null) {
                        vpns.add(vpn);
                    }
                }
            ...
            if (!staticNats.isEmpty()) {
                commandSetupHelper.createApplyStaticNatCommands(staticNats, router, cmds, guestNetworkId);
            }
            ...
            if (!firewallRulesIngress.isEmpty()) {
                commandSetupHelper.createFirewallRulesCommands(firewallRulesIngress, router, cmds, guestNetworkId);
            }

            // Re-apply port forwarding rules
            s_logger.debug("Found " + pfRules.size() + " port forwarding rule(s) to apply as a part of domR " + router + " start.");
            if (!pfRules.isEmpty()) {
                commandSetupHelper.createApplyPortForwardingRulesCommands(pfRules, router, cmds, guestNetworkId);
            }
            ...
            if (!staticNatFirewallRules.isEmpty()) {
                ...
                for (final FirewallRule rule : staticNatFirewallRules) {
                    staticNatRules.add(rulesMgr.buildStaticNatRule(rule, false));
                }
                commandSetupHelper.createApplyStaticNatRulesCommands(staticNatRules, router, cmds, guestNetworkId);
            }
            ...
            final List<LoadBalancerVO> lbs = loadBalancerDao.listByNetworkIdAndScheme(guestNetworkId, Scheme.Public);
            final List<LoadBalancingRule> lbRules = new ArrayList<LoadBalancingRule>();
            if (networkModel.isProviderSupportServiceInNetwork(guestNetworkId, Service.Lb, provider)) {
            ...
            if (!lbRules.isEmpty()) {
                commandSetupHelper.createApplyLoadBalancingRulesCommands(lbRules, router, cmds, guestNetworkId);
            }
        }
        // Reapply dhcp and dns configuration.
        final Network guestNetwork = networkDao.findById(guestNetworkId);
        if (guestNetwork.getGuestType() == GuestType.Shared && networkModel.isProviderSupportServiceInNetwork(guestNetworkId, Service.Dhcp, provider)) {
            final Map<Network.Capability, String> dhcpCapabilities = networkSvc
                    .getNetworkOfferingServiceCapabilities(
                            networkOfferingDao.findById(networkDao.findById(guestNetworkId).getNetworkOfferingId()), Service.Dhcp);
            final String supportsMultipleSubnets = dhcpCapabilities.get(Network.Capability.DhcpAccrossMultipleSubnets);
            if (supportsMultipleSubnets != null && Boolean.valueOf(supportsMultipleSubnets)) {
                final List<NicIpAliasVO> revokedIpAliasVOs = nicIpAliasDao.listByNetworkIdAndState(guestNetworkId, NicIpAlias.State.revoked);
                ...
                final List<NicIpAliasVO> aliasVOs = nicIpAliasDao.listByNetworkIdAndState(guestNetworkId, NicIpAlias.State.active);
                ...
                if (activeIpAliasTOs.size() != 0) {
                    commandSetupHelper.createIpAlias(router, activeIpAliasTOs, guestNetworkId, cmds);
                    commandSetupHelper.configDnsMasq(router, networkDao.findById(guestNetworkId), cmds);
                }
            }
        }
    }
    ...
    private void removeRevokedIpAliasFromDb(final List<NicIpAliasVO> revokedIpAliasVOs) {
        for (final NicIpAliasVO ipalias : revokedIpAliasVOs) {
            nicIpAliasDao.expunge(ipalias.getId());
        }
    }
    ...
    protected void finalizeIpAssocForNetwork(final Commands cmds, final VirtualRouter router, final Provider provider, final Long guestNetworkId,
        ...
        if (publicIps != null && !publicIps.isEmpty()) {
            ...
            if (networkModel.isProviderSupportServiceInNetwork(guestNetworkId, Service.Firewall, provider)) {
                commandSetupHelper.createAssociateIPCommands(router, publicIps, cmds, 0);
            }
        }
    }
    ...
    protected ArrayList<? extends PublicIpAddress> getPublicIpsToApply(final VirtualRouter router, final Provider provider, final Long guestNetworkId,
        ...
        if (guestNetwork.getGuestType() == GuestType.Shared) {
            ...
            userIps = networkModel.listPublicIpsAssignedToGuestNtwk(guestNetworkId, null);
        } else {
            userIps = networkModel.listPublicIpsAssignedToGuestNtwk(ownerId, guestNetworkId, null);
        }
        ...
        if (userIps != null && !userIps.isEmpty()) {
            ...
            for (final IpAddress userIp : userIps) {
                ...
                if (addIp) {
                    final IPAddressVO ipVO = ipAddressDao.findById(userIp.getId());
                    final PublicIp publicIp = PublicIp.createFromAddrAndVlan(ipVO, vlanDao.findById(userIp.getVlanId()));
        ...
        final Network network = networkDao.findById(guestNetworkId);
        final Map<PublicIpAddress, Set<Service>> ipToServices = networkModel.getIpToServices(allPublicIps, false, true);
        final Map<Provider, ArrayList<PublicIpAddress>> providerToIpList = networkModel.getProviderToIpList(network, ipToServices);
    ...
    public boolean finalizeStart(final VirtualMachineProfile profile, final long hostId, final Commands cmds, final ReservationContext context) {
        ...
        for (final Answer answer : cmds.getAnswers()) {
            ...
            if (!answer.getResult()) {
                ...
                alertMgr.sendAlert(AlertService.AlertType.ALERT_TYPE_DOMAIN_ROUTER, router.getDataCenterId(), router.getPodIdToDeployIn(), errorMessage, errorDetails);
        ...
        for (final Nic nic : routerNics) {
            ...
            final DataCenterVO dcVO = dcDao.findById(network.getDataCenterId());
        ...
        if (result) {
            ...
            routerDao.persist(router, guestNetworks);
        }
    ...
    public void finalizeStop(final VirtualMachineProfile profile, final Answer answer) {
        if (answer != null) {
            ...
            final DomainRouterVO domR = routerDao.findById(vm.getId());
    ...
    ResourceUnavailableException {
        ...
        try {
            itMgr.advanceStop(router.getUuid(), forced);
    ...
    public boolean removeDhcpSupportForSubnet(final Network network, final List<DomainRouterVO> routers) throws ResourceUnavailableException {
        ...
        for (final DomainRouterVO router : routers) {
            ...
            final Commands cmds = new Commands(Command.OnError.Continue);
            final List<NicIpAliasVO> revokedIpAliasVOs = nicIpAliasDao.listByNetworkIdAndState(network.getId(), NicIpAlias.State.revoked);
            s_logger.debug("Found" + revokedIpAliasVOs.size() + "ip Aliases to revoke on the router as a part of dhcp configuration");
            ...
            final List<NicIpAliasVO> aliasVOs = nicIpAliasDao.listByNetworkIdAndState(network.getId(), NicIpAlias.State.active);
            s_logger.debug("Found" + aliasVOs.size() + "ip Aliases to apply on the router as a part of dhcp configuration");
            final List<IpAliasTO> activeIpAliasTOs = new ArrayList<IpAliasTO>();
            for (final NicIpAliasVO aliasVO : aliasVOs) {
                activeIpAliasTOs.add(new IpAliasTO(aliasVO.getIp4Address(), aliasVO.getNetmask(), aliasVO.getAliasCount().toString()));
            }
            commandSetupHelper.createDeleteIpAliasCommand(router, revokedIpAliasTOs, activeIpAliasTOs, network.getId(), cmds);
            commandSetupHelper.configDnsMasq(router, network, cmds);
            final boolean result = nwHelper.sendCommandsToRouter(router, cmds);
            if (result) {
                Transaction.execute(new TransactionCallbackNoReturn() {
                    @Override
                    public void doInTransactionWithoutResult(final TransactionStatus status) {
                        for (final NicIpAliasVO revokedAliasVO : revokedIpAliasVOs) {
                            nicIpAliasDao.expunge(revokedAliasVO.getId());
                        }
                    }
                });
                return true;
    ...
    ConcurrentOperationException {
        ...
        final Account owner = accountMgr.getAccount(router.getAccountId());

        // Check if all networks are implemented for the domR; if not -
        // implement them
        final DataCenter dc = dcDao.findById(router.getDataCenterId());
        HostPodVO pod = null;
        if (router.getPodIdToDeployIn() != null) {
            pod = podDao.findById(router.getPodIdToDeployIn());
        }
        ...
        for (final NicVO nic : nics) {
            if (!networkMgr.startNetwork(nic.getNetworkId(), dest, context)) {
        ...
        router = routerDao.findById(routerId);
        if (router.getState() == VirtualMachine.State.Running) {
            return router;
        }
        ...
        final VirtualRouter virtualRouter = nwHelper.startVirtualRouter(router, params);
        if (virtualRouter == null) {
    ...
    public List<VirtualRouter> getRoutersForNetwork(final long networkId) {
        final List<DomainRouterVO> routers = routerDao.findByNetwork(networkId);
    ...
    public void processConnect(final Host host, final StartupCommand cmd, final boolean forRebalance) throws ConnectionException {
        final List<DomainRouterVO> routers = routerDao.listIsolatedByHostId(host.getId());
    ...
    public List<Long> upgradeRouterTemplate(final UpgradeRouterTemplateCmd cmd) {
        ...
        if (domainId != null) {
            ...
            if (accountName != null) {
                final Account account = accountMgr.getActiveAccountByName(accountName, domainId);
                if (account == null) {
    ...
    protected boolean aggregationExecution(final AggregationControlCommand.Action action, final Network network, final List<DomainRouterVO> routers)
        ...
        for (final DomainRouterVO router : routers) {
            ...
            final String routerControlIp = routerControlHelper.getRouterControlIp(router.getId());
            final String routerIpInNetwork = routerControlHelper.getRouterIpInNetwork(network.getId(), router.getId());
