...
public class SecurityGroupManagerImpl2 extends SecurityGroupManagerImpl {
    ...
    //@DB
        ...
        if (_schedulerDisabled) {
            s_logger.debug("Security Group Mgr v2: scheduler disabled, doing nothing for " + affectedVms.size() + " Instances");
            return;
        }
        ...
        if (s_logger.isDebugEnabled()) {
            s_logger.debug("Security Group Mgr v2: scheduling ruleset updates for " + affectedVms.size() + " Instances " + " (unique=" + workItems.size() +
        ...
        p.stop();
        if (s_logger.isDebugEnabled()) {
            s_logger.debug("Security Group Mgr v2: done scheduling ruleset updates for " + workItems.size() + " Instances: num new jobs=" + newJobs +
