...
public class GetUsageRecordsCmd extends BaseListCmd {
    ...
    public void execute(){
        ...
        List<Object> usageRecords = _mgrExt.getUsageRecords(this);
        ...
        for (Object usageRecordGeneric : usageRecords) {
            ...
            } else if (usageRecordGeneric instanceof UsageIPAddressVO) {
                UsageIPAddressVO usageIpAddr = (UsageIPAddressVO)usageRecordGeneric;

                usageRecResponse.setAccountName(ApiDBUtils.findAccountByIdIncludingRemoved(usageIpAddr.getAccountId()).getAccountName());
                usageRecResponse.setAccountId(usageIpAddr.getAccountId());
                usageRecResponse.setDomainId(usageIpAddr.getDomainId());
                usageRecResponse.setZoneId(usageIpAddr.getZoneId());
                usageRecResponse.setDescription("public ip address");
                usageRecResponse.setUsageType(UsageTypes.IP_ADDRESS);
                usageRecResponse.setUsageId(usageIpAddr.getId());
                usageRecResponse.setIpAddress(usageIpAddr.getAddress());
                usageRecResponse.setSourceNat(usageIpAddr.isSourceNat());
                usageRecResponse.setAssignedDate(getDateStringInternal(usageIpAddr.getAssigned()));
                usageRecResponse.setReleasedDate(getDateStringInternal(usageIpAddr.getReleased()));
            }
