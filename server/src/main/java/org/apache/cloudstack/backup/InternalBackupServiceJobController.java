 ...
 * */
public abstract class InternalBackupServiceJobController extends ManagerBase {
    ...
    @Inject
    protected InternalBackupServiceJobDao internalBackupServiceJobDao;
    ...
    protected void rescheduleLostJobs() {
        for (DataCenterVO dataCenterVO : dataCenterDao.listAllZones()) {
            ...
            for (ClusterVO clusterVO : clusterDao.listByDcHyType(dataCenterVO.getId(), Hypervisor.HypervisorType.KVM.toString())) {
                ...
                List<InternalBackupServiceJobVO> lostJobs = getLostJobs(clusterVO, date, hostVOS);
                if (lostJobs.isEmpty()) {
    ...
    protected void processJobResult(InternalBackupServiceJobVO job, boolean result) {
     ...
     */
    protected Pair<HashMap<HostVO, Long>, Integer> getHostToNumberOfExecutingJobsAndTotalExecutingJobs(DataCenterVO zone, InternalBackupServiceJobType... jobTypes) {
        ...
        List<InternalBackupServiceJobVO> executingJobs = internalBackupServiceJobDao.listExecutingJobsByZoneIdAndJobType(zone.getId(), jobTypes);
        for (InternalBackupServiceJobVO executingJob : executingJobs) {
    ...
    protected List<InternalBackupServiceJobVO> thinJobsToStartList(DataCenterVO zone, List<InternalBackupServiceJobVO> jobsToStart, Integer totalExecutingJobs,
     ...
     * */
    protected void submitQueuedJobsForExecution(List<InternalBackupServiceJobVO> jobsToExecute, List<Pair<HostVO, Long>> hostAndNumberOfJobsPairList,
