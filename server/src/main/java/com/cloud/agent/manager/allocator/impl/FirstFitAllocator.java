    ...
    private List<Host> checkHostsCompatibilities(ServiceOffering offering, ExcludeList avoid, List<? extends Host> hosts, int returnUpTo, boolean considerReservedCapacity) {
        ...
        for (Host host : hosts) {
            ...
            if (avoid.shouldAvoid(host)) {
                logger.debug("Host [{}] is in avoid set, skipping this and trying other available hosts",
            ...
            if (cpuCapabilityAndCapacity.first() && cpuCapabilityAndCapacity.second()) {
                ...
                logger.debug("Not using host {}; host has cpu capability? {}, host has capacity? {}.",
                        () -> ReflectionToStringBuilderUtils.reflectOnlySelectedFields(host, "uuid", "name"), cpuCapabilityAndCapacity::first, cpuCapabilityAndCapacity::second);
                avoid.addHost(host.getId());
            }
        }
        return suitableHosts;
