...
@APICommand(name = RemoveTungstenFabricPolicyRuleCmd.APINAME, description = "remove Tungsten-Fabric policy",
...
public class RemoveTungstenFabricPolicyRuleCmd extends BaseAsyncCmd {
    ...
    @Parameter(name = ApiConstants.RULE_UUID, type = CommandType.STRING, required = true, description = "the Uuid of Tungsten-Fabric policy rule")
    private String ruleUuid;

    @Override
    public void execute() throws ResourceUnavailableException, InsufficientCapacityException, ServerApiException,
        ConcurrentOperationException, ResourceAllocationException, NetworkRuleConflictException {
        TungstenFabricPolicyResponse tungstenFabricPolicyResponse = tungstenService.removeTungstenPolicyRule(zoneId,
            policyUuid, ruleUuid);
        if (tungstenFabricPolicyResponse == null) {
            throw new ServerApiException(ApiErrorCode.INTERNAL_ERROR, "Failed to remove Tungsten-Fabric policy rule");
        } else {
    ...
    public String getEventType() {
        return EventTypes.EVENT_TUNGSTEN_REMOVE_POLICY_RULE;
    }

    @Override
    public String getEventDescription() {
        return "remove Tungsten-Fabric policy rule";
    }
