...
public class DirectDownloadManagerImpl extends ManagerBase implements DirectDownloadManager {
    ...
    private void logUsageEvent(VMTemplateVO template, long poolId) {
        ...
        ActionEventUtils.onCompletedActionEvent(CallContext.current().getCallingUserId(), template.getAccountId(), EventVO.LEVEL_INFO, event, description, template.getId(), ApiCommandJobType.Template.toString(), 0);
    }
