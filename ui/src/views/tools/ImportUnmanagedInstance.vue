      ...
      <a-row :gutter="12">
        ...
        <a-col :md="24" :lg="17">
          <a-card :bordered="true">
            <a-form :ref="formRef" :model="form" :rules="rules" @finish="handleSubmit" layout="vertical">
              <a-alert
              ...
              <a-form-item name="hostname" ref="hostname">
                ...
                <a-input v-model:value="form.hostname" :placeholder="apiParams.hostname.description" />
              </a-form-item>
              ...
              <a-form-item
                ...
                ref="templateid"
                v-if="
                  cluster.hypervisortype === 'VMware' ||
                  (cluster.hypervisortype === 'KVM' && !selectedVmwareVcenter && !isDiskImport && !isExternalImport)
                "
              >
                <template #label>
                  <tooltip-label
                    :title="$t('label.templatename')"
                    :tooltip="apiParams.templateid.description + '. ' + $t('message.template.import.vm.temporary')"
                  />
                </template>
                <a-radio-group style="width: 100%" :value="templateType" @change="changeTemplateType">
                  <a-row :gutter="12">
                      ...
                      >
                        <a-select-option v-for="temp in templateSelectOptions" :key="temp.value" :label="temp.label">
                          <span>
                            ...
                            <os-logo
                              v-else-if="temp.value !== null"
                              :osId="temp.ostypeid"
                              :osName="temp.ostypename"
                              size="lg"
              ...
              <a-form-item name="forceconverttopool" ref="forceconverttopool" v-if="selectedVmwareVcenter">
                <template #label>
                  <tooltip-label
                    :title="$t('label.force.convert.to.pool')"
              ...
              >
                <a-select
                  ...
                  @change="
                    (val) => {
                      selectedStoragePoolForConversion = val
                    }
                  "
                >
              ...
              <a-form-item name="forcemstoimportvmfiles" ref="forcemstoimportvmfiles" v-if="selectedVmwareVcenter">
                ...
                <a-switch
                  v-model:checked="form.forcemstoimportvmfiles"
                  @change="
                    (val) => {
                      switches.forceMsToImportVmFiles = val
                    }
                  "
                />
              </a-form-item>
              ...
              <div v-if="resource.disk && resource.disk.length > 1">
                <a-form-item name="selection" ref="selection">
                  <template #label>
                    <tooltip-label
                      :title="$t('label.disk.selection')"
                      :tooltip="apiParams.datadiskofferinglist.description"
                    />
                  </template>
                </a-form-item>
                <a-form-item name="rootdiskid" ref="rootdiskid" :label="$t('label.select.root.disk')">
                  ...
                  >
                    <a-select-option
                      v-for="(opt, optIndex) in resource.disk"
                      :key="optIndex"
                  ...
                  <a-table :columns="selectedRootDiskColumns" :dataSource="selectedRootDiskSources" :pagination="false">
              ...
              <div v-if="resource.nic && resource.nic.length > 0">
                <a-form-item name="networkselection" ref="networkselection">
                  <template #label>
                    <tooltip-label
                      :title="$t('label.network.selection')"
                      :tooltip="apiParams.nicnetworklist.description"
                    />
                  </template>
              ...
              <div v-if="isDiskImport">
                <a-form-item name="networkid" ref="networkid">
                  ...
                  >
                    <a-select-option
                      v-for="network in networkSelectOptions"
                      :key="network.value"
                      :label="network.label"
                    >
                      <span>
              ...
              <a-row v-if="!selectedVmwareVcenter" :gutter="12">
                <a-col :md="24" :lg="12">
                  <a-form-item name="migrateallowed" ref="migrateallowed">
                    <template #label>
                      <tooltip-label
                        :title="$t('label.migrate.allowed')"
                        :tooltip="apiParams.migrateallowed.description"
                      />
                    </template>
                    <a-switch
                      v-model:checked="form.migrateallowed"
                      @change="
                        (val) => {
                          switches.migrateAllowed = val
                        }
                      "
                    />
                  </a-form-item>
    ...
    fetchComputeOfferings (options) {
      ...
      getAPI('listServiceOfferings', {
        ...
        .then((response) => {
          ...
          this.computeOfferings.map((i) => {
            this.offeringsMap[i.id] = i
          })
        })
        .finally(() => {
      ...
      for (var offering of offerings) {
        ...
        if (!offering.iscustomized) {
          ...
          if (offering.serviceofferingdetails) {
            cpuNumberMatches =
              this.resource.cpunumber >= offering.serviceofferingdetails.mincpunumber &&
              this.resource.cpunumber <= offering.serviceofferingdetails.maxcpunumber
            memoryMatches =
              this.resource.memory >= offering.serviceofferingdetails.minmemory &&
    ...
    fetchKvmHostsForConversion () {
      getAPI('listHosts', {
        ...
        this.kvmHostsForConversion.map((host) => {
          ...
          if (
            host.instanceconversionsupported !== null &&
      ...
      this.formRef.value
        ...
        .then(() => {
          ...
          for (var diskId in this.dataDisksOfferingsMapping) {
            ...
            params['datadiskofferinglist[' + diskOfferingIndex + '].diskOffering'] =
              this.dataDisksOfferingsMapping[diskId]
            diskOfferingIndex++
          }
