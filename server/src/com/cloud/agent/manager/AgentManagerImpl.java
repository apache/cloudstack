...
public class AgentManagerImpl implements AgentManager, HandlerFactory {
	...
	public void handleCommands(AgentAttache attache, final long sequence,
			final Command[] cmds) {
		for (Pair<Integer, Listener> listener : _cmdMonitors) {
			boolean processed = listener.second().processCommand(
	...
	public Host findHost(final Host.Type type, final DataCenterVO dc,
			...
			final ServiceOffering offering, final VMTemplateVO template,
		...
		while (en.hasMoreElements()) {
			...
			final Host host = allocator.allocateTo(vmc, offering, type, dc,
					pod, sp, template, avoid);
			if (host == null) {
				continue;
			} else {
				return host;
	...
	public List<HostVO> discoverHosts(long dcId, Long podId, Long clusterId,
			URI url, String username, String password, String hostTags)
		...
		while (en.hasMoreElements()) {
			...
			Map<? extends ServerResource, Map<String, String>> resources = discoverer
					.find(dcId, podId, clusterId, url, username, password);
			if (resources != null) {
				for (Map.Entry<? extends ServerResource, Map<String, String>> entry : resources
						.entrySet()) {
					ServerResource resource = entry.getKey();

					AgentAttache attache = simulateStart(null, resource,
							entry.getValue(), true, hostTagList);
					if (attache != null) {
	...
	public boolean deleteHost(long hostId) {
		...
		try {
			...
			if (host.getType() == Type.Routing
					&& host.getHypervisorType() == Hypervisor.Type.XenServer) {
	...
	protected boolean deleteSecondaryStorageHost(HostVO secStorageHost) {
		...
		try {
			...
			SearchCriteria templateHostSC = _vmTemplateHostDao
			...
			SearchCriteria secStorageCapacitySC = _capacityDao
					.createSearchCriteria();
	...
	@Override
	public Answer send(final Long hostId, final Command cmd, final int timeout)
	...
	@Override
	public Answer[] send(final Long hostId, final Command[] cmds,
			final boolean stopOnError, final int timeout)
			throws AgentUnavailableException, OperationTimedoutException {
	...
	protected Status investigate(AgentAttache agent) {
		...
		try {
			...
			Request req = new Request(seq, hostId, _nodeId,
					new Command[] { new CheckHealthCommand() }, true, true);
	...
	@Override
	public long send(final Long hostId, final Command[] cmds,
			final boolean stopOnError, final Listener listener)
			throws AgentUnavailableException {
		final AgentAttache agent = getAttache(hostId);
		if (agent.isClosed()) {
			return -1;
    ...
    protected AgentAttache simulateStart(Long id, ServerResource resource, Map<String, String> details, boolean old, List<String> hostTags) throws IllegalArgumentException {
        HostVO host = null;
        if (id != null) {
            synchronized (_loadingAgents) {
        ...
        try {
            ...
            if (s_logger.isDebugEnabled()) {
                s_logger.debug("Startup request from directly connected host: " + new Request(0l, -1l, -1l, cmds, true, false).toString());
            }
            try {
	...
	@Override
	public Answer[] send(final Long hostId, final Command[] cmds,
			final boolean stopOnError) throws AgentUnavailableException,
	...
	public boolean maintain(final long hostId) throws AgentUnavailableException {
		...
		if (type == Host.Type.Routing) {
			...
			for (final Long id : ids) {
				...
				if (instance != null
						&& (instance.getState() == State.Running || instance
	...
	public void checkIPConflicts(Host.Type type, HostPodVO pod,
		...
		if (serverPublicIP != null
				&& !_publicIPAddressDao.mark(dc.getId(), serverPublicIP)) {
	...
	public HostVO createHost(final StartupCommand startup,
			...
			boolean directFirst, List<String> hostTags)
			throws IllegalArgumentException {
		Host.Type type = null;
	...
	protected void updateHost(final HostVO host, final StartupCommand startup,
		...
		if (type == Host.Type.Routing) {
			...
			if (scc.getAgentTag() != null
					&& startup.getAgentTag()
							.equalsIgnoreCase("agent-simulator"))
				doCidrCheck = false;
	...
	private void createCapacityEntry(final StartupCommand startup, HostVO server) {
		...
		if ((capacities != null) && !capacities.isEmpty()) {
			for (CapacityVO capacity : capacities) {
				if (capacity.getCapacityType() != CapacityVO.CAPACITY_TYPE_SECONDARY_STORAGE) { // Not
																								// allowing
																								// secondary
																								// storage
																								// to
	...
	protected void upgradeAgent(final Link link, final byte[] request,
			final String reason) {
		...
		assert (reason == UnsupportedVersionException.UnknownVersion) : "Unknown reason: "
				+ reason;
		final UpgradeResponse response = new UpgradeResponse(request,
				_upgradeMgr.getAgentUrl());
		try {
	...
	protected class SimulateStartTask implements Runnable {
		...
		public void run() {
			try {
				...
				simulateStart(id, resource, details, false, null);
			} catch (Exception e) {
	...
	public class AgentHandler extends Task {
		...
		protected void processRequest(final Link link, final Request request) {
			...
			if (attache == null) {
				...
				if ((_upgradeMgr.registerForUpgrade(-1, startup.getVersion()) == UpgradeManager.State.RequiresUpdate)
						&& (_upgradeMgr.getAgentUrl() != null)) {
					final UpgradeCommand upgrade = new UpgradeCommand(
							_upgradeMgr.getAgentUrl());
					final Request req = new Request(1, -1, -1,
							new Command[] { upgrade }, true, true);
					...
					if (cmd instanceof StartupRoutingCommand) {
						...
						if (cmd instanceof PingCommand) {
							...
							if (host == null) {
								...
								s_logger.info("Host "
										+ attache.getId()
										+ " is now ready to processing commands.");
								_hostDao.updateStatus(host, Event.Ready,
										_nodeId);
							}
						} else {
