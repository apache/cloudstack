...
public class ElastistorHostListener implements HypervisorHostListener {
    ...
    public boolean hostConnect(long hostId, long poolId) {
        StoragePool pool = (StoragePool) this.dataStoreMgr.getDataStore(poolId,
        ...
        if (answer == null) {
            throw new CloudRuntimeException(
                    "Unable to get an answer to the modify storage pool command"
                            + pool.getId());
        }

        if (!answer.getResult()) {
            String msg = "Unable to attach storage pool" + poolId
                    + " to the host" + hostId;
            alertMgr.sendAlert(AlertManager.AlertType.ALERT_TYPE_HOST,
                    pool.getDataCenterId(), pool.getPodId(), msg, msg);
            throw new CloudRuntimeException(
                    "Unable establish connection from storage head to storage pool "
                            + pool.getId() + " due to " + answer.getDetails()
                            + pool.getId());
        }
         ...
         * storagePoolHostDao.findByPoolHost(pool.getId(), hostId); if (poolHost
         * == null) { poolHost = new StoragePoolHostVO(pool.getId(), hostId,
         * mspAnswer.getPoolInfo().getLocalPath() .replaceAll("//", "/"));
         * storagePoolHostDao.persist(poolHost); } else {
         * poolHost.setLocalPath(mspAnswer
         * .getPoolInfo().getLocalPath().replaceAll("//", "/")); }
         */
        ...
        s_logger.info("Connection established between " + pool + " host + "
                + hostId);
        return true;
    }
