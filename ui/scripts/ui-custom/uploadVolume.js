...
(function (cloudStack, $) {
    cloudStack.uiCustom.uploadVolume = function (args) {
        ...
        var validate = function ($uploadVolume) {
            ...
            if (!$uploadVolume.find('input[type=radio]:checked, input[type=checkbox]:checked').length) {
                cloudStack.dialog.notice({
        ...
        return function (args) {
            ...
            $uploadVolume.append(topFields, $('<div>').addClass('desc').html(_l('label.select.instance.to.attach.volume.to') + ':'), $('<div>').addClass('listView-container').append(vmList({
                listView: listView
            })));
            $uploadVolume.dialog({
