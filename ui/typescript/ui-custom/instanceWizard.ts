            ...
            var instanceWizard = function (data) {
                ...
                var completeAction = function () {
                    ...
                    $form.find('.my-networks .select .specify-ip input[type=text]').each(function () {
                        var $input = $(this);
                        if (!$input.closest('.select').find('input[type=checkbox]').is(':checked'))
                            return true;
                        data['my-network-ips'].push($input.closest('.select').hasClass('advanced') ?
                            $input.val() : null);
                    });
                    ...
                    if ($form.find('.multi-disk-select-container').length) {
                        ...
                        $selectedDisks.map(function (disk) {
                            data['disk-offerings-multi'].push($.extend($(disk).data('json-obj'), {
                ...
                var makeSelects = function (name, data, fields, options, selectedObj, selectedObjNonEditable) {
                    ...
                    $(data).each(function () {
                        ...
                        var $select = $('<div>')
                            ...
                            .append($('<div>').addClass('select-desc')
                        ...
                        if (options.secondary) {
                            var $secondary = $('<div>').addClass('secondary-input').append($('<input>')
                                .attr({
                                ...
                                .after($('<div>').addClass('name').html(options.secondary.desc))).appendTo($select);
                        }
                    });
                ...
                var dataGenerators = {
                            ...
                            response: {
                                success: function (args) {
                                    ...
                                    $(args.data.zones).each(function (index) {
                                        ...
                                        $step.find('.select-zone select').append($('<option>')
                                            .attr({
                                            value: this.id,
                                            'wizard-field': 'zone'
                                        })
                    ...
                    'select-iso': function ($step, formData) {
                        ...
                        var originalValues = function (formData) {
                            ...
                            $step.find('select[name=hypervisorid]:visible').val(formData.hypervisorid);
                        };
                                    ...
                                    var makeIsos = function (type, append) {
                                        ...
                                        if (type == 'featuredisos' || type == 'communityisos' || type == 'myisos' || type == 'sharedisos') {
                                            ...
                                            $selects.find('input').bind('click', function () {
                                                ...
                                                $select.addClass('selected').append($('<div>').addClass('hypervisor')
                    ...
                    'service-offering': function ($step, formData) {
                        ...
                        return {
                            response: {
                                success: function (args) {
                                    $step.find('.content .select-container').append(makeSelects('serviceofferingid', args.data.serviceOfferings, {
                                        name: 'name',
                                        desc: 'displaytext',
                                        id: 'id'
                                    }, {
                                ...
                                success: function (args) {
                                    ...
                                    if (multiDisk) { // Render as multiple groups for each disk
                                        if (multiDisk[0].id == "none") {
                                            $step.find('.select-container').append($('<p>').addClass('no-datadisk').html(_l('message.no.datadisk')));
                                            return;
                                        }
                                        ...
                                        $(multiDisk).map(function (index, disk) {
                                            ...
                                            var $header = $('<div>').addClass('disk-select-header').append($('<div>').addClass('title').html(disk.label)).appendTo($group);
                                            var $checkbox = $('<input>').addClass('multi-disk-select')
                                                ...
                                                .prependTo($header);
                                            var $multiSelectContainer = $selectContainer.clone().append(makeSelects('diskofferingid.' + disk.id, array_do, {
                                                id: 'id',
                                                name: 'name',
                                                desc: 'displaytext'
                                            }, {
                                        ...
                                        $step.find('.content .select-container').append(makeSelects('diskofferingid', args.data.diskOfferings, {
                                            id: 'id',
                                            name: 'name',
                                            desc: 'displaytext'
                                        }, {
                                    ...
                                    $step.find('input[type=radio]').bind('change', function () {
                                        ...
                                        if (custom && !isMultiDisk) {
                                            $target.parent().find('.name')
                                                .append($('<span>').addClass('custom-size-label')
                                                .append(': ')
                                                .append($('<span>').addClass('custom-disk-size').html($step.find('.custom-size input[name=size]').val()))
                                                .append(' GB'));
                                            ...
                                            $step.find('.section.custom-size').show();
                                            $step.addClass('custom-disk-size');
                                            $target.closest('.select-container').scrollTop($target.position().top);
                                        }
                            ...
                            response: {
                                success: function (args) {
                                    ...
                                    if (args.data.affinityGroups && args.data.affinityGroups.length) {
                                        $step.prepend($('<div>').addClass('main-desc').append($('<p>').html(_l('message.select.affinity.groups'))));
                                        $step.find('.select-container').append(makeSelects('affinity-groups', args.data.affinityGroups, {
                                            name: 'name',
                                            desc: 'description',
                                            id: 'id'
                                        }, {
                                        ...
                                        }, args.data.selectedObj, args.data.selectedObjNonEditable));
                                    }
                                    else {
                                        $step.find('.select-container').append($('<p>').addClass('no-affinity-groups').html(_l('message.no.affinity.groups')));
                                    }
                                }
                            }
                        };
                            ...
                            response: {
                                success: function (args) {
                                    ...
                                    if (args.data.sshkeyPairs && args.data.sshkeyPairs.length) {
                                        $step.prepend($('<div>').addClass('main-desc').append($('<p>').html(_l('message.please.select.ssh.key.pair.use.with.this.vm'))));
                                        $step.find('.section.no-thanks').show();
                                        $step.find('.select-container').append(makeSelects('sshkeypair', args.data.sshkeyPairs, {
                                            name: 'name',
                                            id: 'name'
                                        }, {
                                        ...
                                        $step.find('.section.no-thanks').hide();
                                        $step.find('.select-container').append($('<p>').addClass('no-sshkey-pairs').html(_l('You do not have any ssh key pairs. Please continue to the next step.')));
                                    }
                                }
                            }
                        };
                        ...
                        return {
                            response: {
                                success: function (args) {
                                    ...
                                    // My networks
                                    $step.find('.my-networks .select-container').append(makeSelects('my-networks', args.data.networkObjs, {
                                        name: 'name',
                                        desc: 'type',
                                        id: 'id'
                                    }, {
                                    ...
                                    $step.find('.my-networks .select-container .select, .select.new-network .select').each(function () {
                                        ...
                                        var $specifyIpField = $('<div>').addClass('specify-ip hide-if-unselected').append($('<label>').html(_l('label.ip.address')), $('<input>').attr({ type: 'text' }));
                                        // Cleanup
                ...
                var showStep = function (index, options) {
                    ...
                    if (!$targetStep.hasClass('loaded')) {
                        ...
                        dataProvider(index, dataGenerators[stepID]($targetStep, formData));
                        if (!$targetStep.hasClass('repeat') && !$targetStep.hasClass('always-load'))
                ...
                $wizard.click(function (event) {
                    ...
                    if ($target.closest('div.button.next').length) {
                        ...
                        if ($activeStep.find('.wizard-step-conditional.select-network:visible').length > 0) {
                            ...
                            if (!data) {
                                $activeStep.closest('form').data('my-networks', cloudStack.serializeForm($activeStep.closest('form'))['my-networks']);
                            }
                ...
                $wizard.find('.slider').each(function () {
                    ...
                    $slider.slider({
                        ...
                        slide: function (event, ui) {
                            $slider.closest('.section.custom-size').find('input[type=text]').val(ui.value);
