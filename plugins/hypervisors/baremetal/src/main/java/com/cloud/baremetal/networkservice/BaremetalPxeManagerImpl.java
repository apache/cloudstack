...
public class BaremetalPxeManagerImpl extends ManagerBase implements BaremetalPxeManager, ResourceStateAdapter {
    ...
    public boolean addUserData(NicProfile nic, VirtualMachineProfile profile) {
        ...
        try {
            ...
            if (!ans.getResult()) {
                logger.debug(String.format("Add User Data to Instance:%s failed because %s", vm.getInstanceName(), ans.getDetails()));
                return false;
            } else {
                return true;
            }
        } catch (Exception e) {
            logger.debug(String.format("Add User Data to Instance:%s failed", vm.getInstanceName()), e);
            return false;
        }
    }
