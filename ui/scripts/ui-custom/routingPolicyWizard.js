(function($, cloudStack) {
    ...
    cloudStack.uiCustom.routingPolicyWizard = function(args) {

        return function() {
            var $wizard = $('#template').find('div.routing-policy-wizard').clone();
            ...
            var showStep = function(index, goBack, options) {
                ...
                if (typeof index == 'string') {
                    index = $wizard.find('[routing-policy-wizard-step-id=' + index + ']').index() + 1;
                }
                ...
                var formID = $targetStep.attr('routing-policy-wizard-form');
                var stepPreFilter = cloudStack.routingPolicyWizard.preFilters[
                    $targetStep.attr('routing-policy-wizard-prefilter')
                    ];
                ...
                if ($targetStep.index() == $steps.length - 1 || options.nextStep) {
                    $nextButton.find('span').html(options.nextStep ? _l('label.save.changes') : _l('label.tungsten.create.routing.policy'));
            ...
            var addRoutingPolicy = function (args, terms) {
                ...
                var routingPolicy = createRoutingPolicy(args.zoneId, prefixList.routingPolicy.name);
                addRoutingPolicyTerm(routingPolicy.uuid, args.zoneId, terms);
            };

            var createRoutingPolicy = function (zoneid, name) {
                ...
                $.ajax({
                    url: createURL('createTungstenFabricRoutingPolicy'),
                    data: dataObj,
                    async: false,
                    success: function (json) {
                        routingPolicy = json.createtungstenfabricroutingpolicyresponse.routingpolicy;
                    }
                });
