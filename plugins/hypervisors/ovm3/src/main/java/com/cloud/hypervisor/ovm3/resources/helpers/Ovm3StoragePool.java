...
public class Ovm3StoragePool {
    ...
    public boolean prepareForPool() throws ConfigurationException {
        ...
        try {
            ...
            if (host.getServerRoles().contentEquals(
                ...
                LOGGER.debug("Server role for host " + c.getHostname()
                        + " is ok");
            } else {
                try {
                    ...
                    LOGGER.debug(
                            "Failed to set server role for host "
                                    + c.getHostname(), e);
            ...
            if (host.getMembershipState().contentEquals("Unowned")) {
                try {
                    LOGGER.debug("Take ownership of host " + c.getHostname());
                ...
                if (host.getManagerUuid().equals(agentOwnedByUuid)) {
                    String msg = "Host " + c.getHostname() + " owned by us";
                    LOGGER.debug(msg);
                    return true;
                } else {
                    String msg = "Host " + c.getHostname()
            ...
            String msg = "Failed to prepare " + c.getHostname() + " for pool";
    ...
    public Boolean setupPool(StorageFilerTO cmd) throws Ovm3ResourceException {
        ...
        if (agentIsMaster) {
            try {
                LOGGER.debug("Create poolfs on " + c.getHostname()
                        + " for repo " + primUuid);
            ...
            try {
                poolHost.createServerPool(poolAlias, primUuid, ovm3PoolVip,
                        poolSize + 1, c.getHostname(), c.getIp());
            } catch (Ovm3ResourceException e) {
                throw e;
            }
    ...
    public Boolean addMembers() throws Ovm3ResourceException {
        ...
        try {
            Connection m = new Connection(ovm3PoolVip, c.getPort(),
            ...
            if (poolMaster.isInAPool()) {
                ...
                LOGGER.warn(c.getIp() + " noticed master " + ovm3PoolVip
                        + " is not part of pool");
                return false;
            }
        ...
        if (cmd.getType() == StoragePoolType.NetworkFilesystem) {
            ...
            try {
                ...
                LOGGER.debug("Unable to mount NFS repository " + mountPoint
                        + " on " + ovsRepo + " requested for " + c.getHostname()
                        + ": " + e.getMessage());
            }
            try {
    ...
    public Answer execute(CreateStoragePoolCommand cmd) {
        ...
        try {
            ...
            String msg = "Catch Exception " + e.getClass().getName()
                    ...
                    + " on host:" + c.getHostname() + " pool: " + pool.getHost()
                    + pool.getPath();
            LOGGER.warn(msg, e);
