...
@APICommand(name = RemoveTungstenFabricRoutingPolicyTermCmd.APINAME, description = "remove Tungsten-Fabric static route from interface",
        responseObject = SuccessResponse.class, requestHasSensitiveInfo = false,
        responseHasSensitiveInfo = false)
public class RemoveTungstenFabricRoutingPolicyTermCmd extends BaseAsyncCmd {
    public static final Logger s_logger = Logger.getLogger(RemoveTungstenFabricRoutingPolicyTermCmd.class.getName());
    public static final String APINAME = "removeTungstenFabricRoutingPolicyTerm";
    ...
    public void execute() throws ResourceUnavailableException, InsufficientCapacityException, ServerApiException, ConcurrentOperationException, ResourceAllocationException, NetworkRuleConflictException {
        boolean result = tungstenService.removeRoutingPolicyTerm(zoneId, routingPolicyUuid, communities, matchAll, protocolList, prefixList);
        if(result){
            SuccessResponse successResponse = new SuccessResponse(getCommandName());
            this.setResponseObject(successResponse);
        } else {
            throw new ServerApiException(ApiErrorCode.INTERNAL_ERROR, "Failed to remove Tungsten-Fabric routing policy term");
        }
    }

    @Override
    public String getEventType() {
        return EventTypes.EVENT_TUNGSTEN_REMOVE_ROUTING_POLICY_TERM;
    }

    @Override
    public String getEventDescription() {
        return "Remove Tungsten-Fabric routing policy term";
    }
