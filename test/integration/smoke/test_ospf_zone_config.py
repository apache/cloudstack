...
class TestQuagga(cloudstackTestCase):
    ...
    def updateQuaggaConfig(self, enabled, cidr_list, password):
        ...
        cmd.zoneid = self.zone.id
        cmd.ospfSuperCIDR = cidr_list
        cmd.quaggaEnabled = enabled
        cmd.quaggaPassword = password
        ...
        self.assertEqual(
                         response.quaggaEnabled, enabled
                         )
        
        
        self.assertEqual(
                         response.quaggaPassword, password
                         )
        
        self.assertEqual(
                         response.ospfSuperCIDR, cidr_list
                         )

    @attr(tags=["smoke", "advanced"], required_hardware="false")
    def test_01_quaggaConfig(self):
        cidr_lsit="200.100.0.0/18,216.100.0.0/20"
        password="6P33lsadEbmiJ7ZN7gK8vylOY2dTza3rAuY8UFboH7I=:Xb1G4VhzBkSXmTa8vDqExQQo8PiIUUSNtpxmdOkX1z8="
        self.updateQuaggaConfig("true", cidr_list, password)
        return
    ...
    def test_02_quaggaConfig(self):
        cidr_lsit="200.100.0.0/18,216.100.0.0/20"
        password="6P33lsadEbmiJ7ZN7gK8vylOY2dTza3rAuY8UFboH7I=:Xb1G4VhzBkSXmTa8vDqExQQo8PiIUUSNtpxmdOkX1z8="
        self.updateQuaggaConfig("false", cidr_list, password)
