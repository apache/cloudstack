...
public class LibvirtComputingResource extends ServerResourceBase implements ServerResource, VirtualRouterDeployer, ResourceStatusUpdater {
    ...
    public PowerState getVmState(final Connect conn, final String vmName) {
        ...
        while (retry-- > 0) {
            try {
                ...
                s_logger.error(String.format("Can't get state for VM [%s] (retry=%s).", vmName, retry), e);
            } finally {
                try {
                    if (vms != null) {
                        vms.free();
    ...
    public VmStatsEntry getVmStat(final Connect conn, final String vmName) throws LibvirtException {
        Domain dm = null;
        try {
            s_logger.debug(String.format("Trying to get VM with name [%s].", vmName));
            dm = getDomain(conn, vmName);
            if (dm == null) {
                s_logger.warn(String.format("Could not get VM with name [%s].", vmName));
                return null;
            }
            ...
            s_logger.info(String.format("Saving stats for VM [%s].", vmAsString));
            vmStats.put(vmName, newStats);
            s_logger.debug(String.format("Saved stats for VM [%s]: [%s].", vmAsString, newStats));
    ...
    protected LibvirtExtendedVmStatsEntry getVmCurrentStats(final Domain dm) throws LibvirtException {
        ...
        s_logger.debug(String.format("Retrieved statistics for VM [%s]: [%s].", vmToString(dm), stats));
    ...
    protected void getVmCurrentCpuStats(final Domain dm, final LibvirtExtendedVmStatsEntry stats) throws LibvirtException {
        s_logger.debug(String.format("Getting CPU stats for VM [%s].", vmToString(dm)));
    ...
    protected void getVmCurrentNetworkStats(final Domain dm, final LibvirtExtendedVmStatsEntry stats) throws LibvirtException {
        ...
        s_logger.debug(String.format("Getting network stats for VM [%s].", vmAsString));
        final List<InterfaceDef> vifs = getInterfaces(dm.getConnect(), dm.getName());
        s_logger.debug(String.format("Found [%d] network interface(s) for VM [%s].", vifs.size(), vmAsString));
        double rx = 0;
        double tx = 0;
    ...
    protected void getVmCurrentDiskStats(final Domain dm, final LibvirtExtendedVmStatsEntry stats) throws LibvirtException {
        final String vmAsString = vmToString(dm);
        s_logger.debug(String.format("Getting disk stats for VM [%s].", vmAsString));
        final List<DiskDef> disks = getDisks(dm.getConnect(), dm.getName());
        s_logger.debug(String.format("Found [%d] disk(s) for VM [%s].", disks.size(), vmAsString));
        long io_rd = 0;
        long io_wr = 0;
        double bytes_rd = 0;
        double bytes_wr = 0;
        for (final DiskDef disk : disks) {
            if (disk.getDeviceType() == DeviceType.CDROM || disk.getDeviceType() == DeviceType.FLOPPY) {
                s_logger.debug(String.format("Ignoring disk [%s] in VM [%s]'s stats since its deviceType is [%s].", disk.toString().replace("\n", ""), vmAsString, disk.getDeviceType()));
                continue;
            }
            final DomainBlockStats blockStats = dm.blockStats(disk.getDiskLabel());
    ...
    protected VmStatsEntry calculateVmMetrics(final Domain dm, final LibvirtExtendedVmStatsEntry oldStats, final LibvirtExtendedVmStatsEntry newStats) throws LibvirtException {
        ...
        s_logger.debug(String.format("Writing VM [%s]'s CPU and memory information into the metrics.", vmAsString));
        metrics.setNumCPUs(info.nrVirtCpu);
        metrics.setMemoryKBs(info.maxMem);
        metrics.setTargetMemoryKBs(info.memory);
        s_logger.debug(String.format("Trying to get free memory for VM [%s].", vmAsString));
        ...
        if (oldStats != null) {
            ...
            s_logger.debug(String.format("Calculating network utilization for VM [%s].", vmAsString));
            ...
            s_logger.debug(String.format("Calculating disk utilization for VM [%s].", vmAsString));
        ...
        s_logger.debug(String.format("Calculated metrics for VM [%s]: [%s].", vmAsString, metricsAsString));
    ...
    protected long getMemoryFreeInKBs(Domain dm) throws LibvirtException {
        ...
        if(s_logger.isTraceEnabled()){
            s_logger.trace(String.format("Retrieved memory statistics (information about tags can be found on the libvirt documentation): [%s].", ArrayUtils.toString(memoryStats)));
        }
