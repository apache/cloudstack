...
public class DataMigrationUtility {
     ...
     */
    public boolean filesReadyToMigrate(Long srcDataStoreId, List<TemplateDataStoreVO> templates, List<SnapshotDataStoreVO> snapshots, List<VolumeDataStoreVO> volumes, List<NativeBackupJoinVO> backups) {
    ...
    private boolean filesReadyToMigrate(Long srcDataStoreId) {
        ...
        List<NativeBackupJoinVO> backups = nativeBackupJoinDao.listByImageStoreId(srcDataStoreId);
    ...
    protected List<DataObject> getAllReadyBackupsAndChains(DataStore srcDataStore, Map<DataObject, Pair<List<List<BackupObject>>, Long>> backupChains) {
        List<NativeBackupJoinVO> backups = nativeBackupJoinDao.listByImageStoreId(srcDataStore.getId());
        return getAllReadyBackupsAndChains(backupChains, backups);
    }

    private List<DataObject> getAllReadyBackupsAndChains(Map<DataObject, Pair<List<List<BackupObject>>, Long>> backupsChains, List<NativeBackupJoinVO> backups) {
        Set<Long> backupIdsToMigrate = backups.stream().map(NativeBackupJoinVO::getId).collect(Collectors.toSet());
        List<List<BackupObject>> backupChains;
        Set<Long> backupIdsAlreadyInChain = new HashSet<>();
    ...
    private List<List<BackupObject>> createBackupChain(NativeBackupJoinVO backup) {
        ...
        chain.add(nativeBackupJoinDao.listById(backup.getId()).stream().map(BackupObject::getBackupObject).collect(Collectors.toList()));
