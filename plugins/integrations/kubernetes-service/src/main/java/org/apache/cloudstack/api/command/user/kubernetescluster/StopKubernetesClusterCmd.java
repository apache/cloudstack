...
public class StopKubernetesClusterCmd extends BaseAsyncCmd {
    ...
    public void execute() throws ResourceUnavailableException, InsufficientCapacityException, ServerApiException, ConcurrentOperationException, ResourceAllocationException, NetworkRuleConflictException {
        ...
        try {
            final boolean result = kubernetesClusterService.stopKubernetesCluster(getId());
            final SuccessResponse response = new SuccessResponse(getCommandName());
            response.setSuccess(result);
            setResponseObject(response);
        } catch (ManagementServerException ex) {
            LOGGER.warn("Failed to stop Kubernetes cluster:" + kubernetesCluster.getUuid() + " due to " + ex.getMessage());
            throw new ServerApiException(ApiErrorCode.INTERNAL_ERROR,
                    "Failed to stop Kubernetes cluster:" + kubernetesCluster.getUuid(), ex);
        }
    }
