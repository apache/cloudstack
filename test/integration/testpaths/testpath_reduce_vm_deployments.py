...
class TestDeployVMandVMLifeCycle(cloudstackTestCase):
    ...
    def test_reset_vm_on_reboot(self):
        # TODO: SIMENH: add new test to check volume contents
        """Test reset virtual machine on reboot
        """
        # Validate the following
        # create vm and list the volume for that VM. Reboot vm and check if
        # root volume is different as before.
        ...
        self.debug(
            "volume before and after rebbot %s....%s" %
            (volume_after_reboot.id, volume_before_reboot.id))
        self.assertNotEqual(
    ...
    def test_snapshot_root_disk(self):
        ...
        self.debug(
            "select backup_snap_id, account_id, volume_id from snapshots where uuid = '%s';" % str(
                snapshot.id))
    ...
    def test_04_change_offering_small(self):
        ...
        if self.hypervisor.lower() == "lxc":
            self.skipTest(
                "Skipping this test for {} due to bug CS-38153".format(self.hypervisor))
        try:
    ...
    def test_02_deploy_vm_root_resize(self):
        ...
        if (self.hypervisor.lower() == 'kvm'):
            ...
            try:
                ...
                if "rootdisksize override is smaller than template size" in str(
                        ex):
                    success = True
                else:
                    self.debug(
            ...
            self.assertEqual(
                success,
                True,
                "Check if passing rootdisksize < templatesize fails appropriately")
        else:
            self.debug(
                "test 01 does not support hypervisor type " +
                self.hypervisor)
