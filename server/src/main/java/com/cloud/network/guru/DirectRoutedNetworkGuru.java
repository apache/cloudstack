...
public class DirectRoutedNetworkGuru extends DirectNetworkGuru {
    ...
    /**
     * Designs the network with {@code Mode.Static} (Instances are configured via ConfigDrive,
     * there is no DHCP) and a {@code BroadcastDomainType.Routed} broadcast domain: routed://&lt;id&gt;
     * names the per-network bridge (brdr-&lt;id&gt;), consuming no VLAN/VXLAN and nothing on the wire.
     * The routed id — operator-specified or allocated from the physical network's vnet range —
     * arrives as the broadcast URI, stamped by the orchestrator before design, and never changes:
     * the bridge name derives from it. The IPv4/IPv6 subnets are allocation pools routed to the
     * hypervisors, not broadcast domains, and carry no gateways of their own: the Instance's
     * gateway is always the shared link-local address, so a subnet gateway would only burn an
     * address.
     */
    @Override
     ...
     * agent recognises a direct routed NIC. The inherited allocation likewise labels the NIC's
     * isolation URI vlan://&lt;tag&gt; from the vlan row; there is no VLAN here — the only
     * isolation-shaped fact about this NIC is its routed://&lt;id&gt; broadcast domain, so that
     * replaces the isolation URI as well.
     */
