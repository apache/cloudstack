...
export default {
  ...
  methods: {
    ...
    async stepAddPrimaryStorage () {
      ...
      if (protocol === 'nfs') {
        let path = this.prefillContent.primaryStoragePath ? this.prefillContent.primaryStoragePath.value : ''
        ...
        params['details[0].user'] = this.prefillContent.primaryStorageSMBUsername ? this.prefillContent.primaryStorageSMBUsername.value : null
        params['details[0].password'] = this.prefillContent.primaryStorageSMBPassword ? this.prefillContent.primaryStorageSMBPassword.value : null
        params['details[0].domain'] = this.prefillContent.primaryStorageSMBDomain ? this.prefillContent.primaryStorageSMBDomain.value : null
      } else if (protocol === 'PreSetup') {
        let path = ''
        if (this.stepData.clusterReturned.hypervisortype === 'XenServer') {
          path = this.prefillContent.primaryStorageSRLabel ? this.prefillContent.primaryStorageSRLabel.value : ''
          server = 'localhost'
        } else {
          path = this.prefillContent.primaryStoragePath ? this.prefillContent.primaryStoragePath.value : ''
        }
        if (path.substring(0, 1) !== '/') {
          path = '/' + path
        }
        ...
        let vg = this.prefillContent.primaryStorageVolumeGroup ? this.prefillContent.primaryStorageVolumeGroup.value : ''
        if (vg.substring(0, 1) !== '/') {
          vg = '/' + vg
        }
        url = this.clvmURL(vg)
      } else if (protocol === 'rbd') {
        const rbdmonitor = this.prefillContent.primaryStorageRADOSMonitor ? this.prefillContent.primaryStorageRADOSMonitor.value : ''
        const rbdpool = this.prefillContent.primaryStorageRADOSPool ? this.prefillContent.primaryStorageRADOSPool.value : ''
        const rbdid = this.prefillContent.primaryStorageRADOSUser ? this.prefillContent.primaryStorageRADOSUser.value : ''
        const rbdsecret = this.prefillContent.primaryStorageRADOSSecret ? this.prefillContent.primaryStorageRADOSSecret.value : ''
        url = this.rbdURL(rbdmonitor, rbdpool, rbdid, rbdsecret)
      } else if (protocol === 'vmfs') {
        let path = this.prefillContent.primaryStorageVmfsDatacenter ? this.prefillContent.primaryStorageVmfsDatacenter.value : ''
        if (path.substring(0, 1) !== '/') {
          path = '/' + path
        }
        path += '/' + this.prefillContent.primaryStorageVmfsDatastore ? this.prefillContent.primaryStorageVmfsDatastore.value : ''
        url = this.vmfsURL('dummy', path)
      } else {
        let iqn = this.prefillContent.primaryStorageTargetIQN ? this.prefillContent.primaryStorageTargetIQN.value : ''
        if (iqn.substring(0, 1) !== '/') {
          iqn = '/' + iqn
        }
        const lun = this.prefillContent.primaryStorageLUN ? this.prefillContent.primaryStorageLUN.value : ''
        url = this.iscsiURL(server, iqn, lun)
      }

      params.url = url
      params.tags = this.prefillContent.primaryStorageTags ? this.prefillContent.primaryStorageTags.value : ''
