...
public class S3PolicyStatement {
    ...
    public boolean containsResource(String pathToObject) {
        ...
        if (pathToObject.matches(regexResource))
            return true;
    ...
    public String toString() {
        ...
        if (null != sid)
            value.append("Sid: " + sid + "\n");
        if (null != effect)
            value.append("Effect: " + effect + "\n");
        if (null != principals)
            value.append(principals.toString());
        if (null != actions)
            value.append(actions.toString());
        if (null != notAction)
            value.append("NotAction: " + notAction + "\n");
        if (null != resource)
            value.append("Resource: " + resource + "\n");
        if (null != regexResource)
            value.append("Regex Resource: " + regexResource + "\n");
        if (null != block)
