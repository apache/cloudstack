...
(function ($, cloudStack, _l) {
    var replaceListViewItem = function ($detailView, newData, options) {
        ...
        if (!$listView.parents('html').length)
            return;
        var $newRow;
    ...
    var uiActions = {
        ...
        standard: function ($detailView, args, additional) {
            ...
            var context = $.extend(true, {}, args.context ? args.context : $detailView.data('view-args').context);
            ...
            var updateTabContent = function (newData) {
                ...
                makeTabs($detailView, $detailView.data('view-args').tabs, {
                    context: context,
                    ...
                    action.action({
                        ...
                        response: {
                            success: function (args) {
                                ...
                                // Setup notification
                                cloudStack.ui.notifications.add(notification, function (args2) {
                                    //name parameter as "args2" instead of "args" to avoid override "args" from success: function (args) {
                                    if ($detailView.parents('html').length) {
                                        $loading.remove();
                            ...
                            error: function (args) {
                                //args here is parsed errortext from API response
                    ...
                    if (isMultiple && $detailView.is(':visible')) {
                        ...
                        if ($detailView.data("list-view-row") != null) {
                            ...
                            if (!$tbody.find('tr').length) {
                                $("<tr>").addClass('empty').append($("<td>").html(_l('label.no.data'))).appendTo($tbody);
                            }
        ...
        edit: function ($detailView, args) {
            ...
            var token_value = "";
            if ($detailView.find('.button.done').length)
                return false;
            // Convert value TDs
            ...
            $.merge($editButton, $cancelButton)
                .appendTo($detailView.find('.ui-tabs-panel .detail-group.actions')).fadeIn();
            ...
            var convertInputs = function ($inputs) {
                ...
                $inputs.each(function () {
                    ...
                    if ($input.is('input[type=text]')) {
                        ...
                        $value.html(_s($input.attr('value')));
                    }
                        ...
                        response: {
                            success: function (args) {
                                ...
                                if (!action.notification) {
                                    ...
                                    cloudStack.ui.notifications.add(notificationArgs, function () { }, []);
                                    replaceListViewItem($detailView, data);
                                    removeEditForm();
                                }
                                else {
                                    $loading.appendTo($detailView);
                                    cloudStack.ui.notifications.add($.extend(true, {}, notificationArgs, action.notification), function (args) {
                                        replaceListViewItem($detailView, data);
                                        convertInputs($inputs);
                                        removeEditForm();
                                        $loading.remove();
            ...
            $detailView.find('td.value span').each(function () {
                ...
                var $value = $(this);
                if (!$value.data('detail-view-is-editable'))
                    return true;
            ...
            if ($detailView.find('td.value span:data(detail-view-is-editable)').length) {
                var $detailsEdit = $detailView.find('div.main-groups').detach(), $detailsEditForm = $('<form>').append($detailsEdit);
        ...
        if (isCustom) {
            $browser.cloudBrowser('addPanel', {
                ...
                complete: function ($newPanel) {
                    $newPanel.append(viewAllID.custom({
                        $browser: $browser,
                        context: $detailView.data('view-args').context,
    ...
    var makeActionButtons = function (actions, options) {
        ...
        var $actions = $('<td>').addClass('detail-actions').append($('<div>').addClass('buttons'));
        var allowedActions = [];
        if (actions) {
            ...
            $.each(actions, function (key, value) {
                ...
                if (value.textLabel || options.compact) {
                    $action
                        ...
                        .prepend($('<span>').addClass('label').html(_l(options.compact ?
                        (value.compactLabel ?
        ...
        return $('<div>')
            ...
            .append($('<table>').append($('<tbody>').append($('<tr>').append($actions))));
    };
    /**
    ...
    var makeFieldContent = function (tabData, $detailView, data, args) {
        ...
        $(fields).each(function () {
            var fieldGroup = this;
            var $detailTable = $('<tbody></tbody>').appendTo($('<table></table>').appendTo($('<div></div>').addClass('detail-group').appendTo($detailGroups.find('.main-groups'))));
            ...
            if (detailViewArgs.viewAll && showViewAll) {
                if (!$.isArray(detailViewArgs.viewAll)) {
                    $('<div>')
                        ...
                        .append($('<span>').html(_l('label.view') + ' ' + _l(detailViewArgs.viewAll.label))))
                    ...
                    $(detailViewArgs.viewAll).each(function () {
                        ...
                        $('<div>')
                            ...
                            .append($('<span>').html(_l('label.view') + ' ' + _l(viewAllItem.label))))
                            .append($('<div>').addClass('end'))
                            .appendTo($('<td>')
    ...
    var loadTabContent = function ($tabContent, args, options) {
        ...
        $.extend($detailView.data('view-args'), {
            activeTab: targetTabID
        });
        if (!$detailView.data('view-args').compact) {
            $tabContent.append($('<div>').addClass('loading-overlay'));
        }
        return dataProvider({
            tab: targetTabID,
            id: args.id,
            ...
            response: {
                success: function (args) {
                    ...
                    if (isMultiple) {
                        $(data).each(function () {
                            var item = this;
                            var $fieldContent = makeFieldContent($.extend(true, {}, tabs, {
                                id: targetTabID
                            ...
                            if (tabData.viewAll) {
                                $fieldContent.find('tr')
                                    ...
                                    .append($('<div>').addClass('view-all').append($('<span>').html(tabData.viewAll.label ?
                                    _l(tabData.viewAll.label) :
                                    _l('label.view.all')), $('<div>').addClass('end')).click(function () {
                                    viewAll(tabData.viewAll.path, {
                        ...
                        if (tabData.multiple && tabData.actions && tabData.actions.add) {
                            $tabContent.prepend($('<div>').addClass('button add').append($('<span>').addClass('icon').html('&nbsp;'), $('<span>').html(_l(tabData.actions.add.label))).click(function () {
                                uiActions.standard($detailView, {
                                    tabs: tabList,
                                    activeTab: targetTabID,
                                    ...
                                    complete: function (args) {
                                        if ($detailView.is(':visible')) {
                                            loadTabContent($detailView.find('div.detail-group:visible'), $detailView.data('view-args'));
                                        }
                                    }
                                });
                            }));
                    ...
                    if (tabs.tags &&
                        ...
                        $('<div>').tagger($.extend(true, {}, tabs.tags, {
                            context: $detailView.data('view-args').context,
    ...
    var makeTabs = function ($detailView, tabs, options) {
        ...
        if (options.newData &&
            ...
            $.extend(context[$detailView.data('view-args').section][0], options.newData);
        }
        if (tabFilter && !compact) {
            ...
            removedTabs = $.grep($.map(tabs, function (value, key) {
                return key;
            }), function (tab, index) {
                return index > 0;
            });
        ...
        return $.merge($tabs, $tabContentGroup.children());
    };
    ...
    var makeToolbar = function () {
        return $('<div class="toolbar">')
            ...
            .append($('<span>').html(_l('label.refresh'))));
    };
    $.fn.detailView = function (args, options) {
        ...
        $detailView.tabs($.extend(true, {}, tabEvents, {
            select: function () {
    ...
    var manageTabsContent = function (event, ui) {
        ...
        if (!$target.hasClass('detail-view') || $target.hasClass('detail-view ui-state-active'))
            return true;
    ...
    $(document).bind('click', function (event) {
        ...
        if ($target.closest('div.toolbar div.refresh').length) {
            loadTabContent($target.closest('div.detail-view').find('div.detail-group:visible'), $target.closest('div.detail-view').data('view-args'), { refresh: true });
            return false;
        }
        // Detail action
    ...
    $(window).bind('cloudStack.detailsRefresh', function () {
        ...
        $detailView.each(function () {
            var $detailView = $(this), args = $detailView.data('view-args');
