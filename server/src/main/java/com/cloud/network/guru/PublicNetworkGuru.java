...
public class PublicNetworkGuru extends AdapterBase implements NetworkGuru {
    ...
    /**
     * On a direct routed public range (SystemVMs on a ROUTED physical network) the address is a
     * host route, not a subnet membership, and the NIC takes the same form as a guest NIC on a
     * direct routed network: /32 with the shared on-link gateway, and the routed://&lt;id&gt;
     * broadcast domain that names the bridge on the host. The MAC and format are set before the
     * branch on the range type, because setRoutedRangeIpv6() derives the EUI-64 address from the
     * MAC and upgrades the format to DualStack.
     */
