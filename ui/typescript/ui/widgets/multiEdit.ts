...
(function ($, cloudStack) {
    ...
    var _medit = cloudStack.ui.widgets.multiEdit = {
        ...
        addItem: function (data, fields, $multi, itemData, actions, options) {
                    ...
                    options.itemActions.add.action({
                        ...
                        response: {
                            success: function (args) {
                                ...
                                cloudStack.ui.notifications.add(args.notification, complete, {}, notificationError, {});
                            },
                            error: error
                        }
                    });
                ...
                if (!field.isPassword) {
                    ...
                    if (field.edit) {
                        ...
                        if (selectPermission) {
                            ...
                            $matchingSelect.find('option').each(function () {
                        ...
                        if (options.multipleAdd) {
                            $addButton.click(function () {
                                ...
                                _medit.vmList($multi, options.listView, context, options.multipleAdd, _l('label.add.vms'), addItemAction, {
                                    multiRule: multiRule
                                });
                                return true;
                            });
            ...
            $.each(actions, function (actionID, action) {
                if (allowedActions && $.inArray(actionID, allowedActions) == -1)
                    return true;
                $actions.append($('<div>').addClass('action')
                    .addClass(actionID)
                                            ...
                                            poll: function (args) {
                                                ...
                                                notification.poll({
                                                    ...
                                                    complete: function (args) {
                                                        ...
                                                        complete();
                                                        if (actionOptions.complete)
                                                            actionOptions.complete();
                                                    },
                                                    error: function (args) {
                                                        error(args);
                                                        $multi.trigger('refresh');
                    ...
                    if (!isEdit) {
                        ...
                        $.each(fields, function (key, field) {
                            ...
                            if (field && field.isEditable)
                                editableFields[key] = $.extend(true, {}, field, {
            ...
            if (options.tags) {
                $actions.prepend($('<div></div>')
                    ...
                    }).append($('<div></div>').addClass('multi-edit-tags').tagger($.extend(true, {}, options.tags, {
                        context: $.extend(true, {}, options.context, {
                            multiRule: [multiRule]
                        })
                    })));
            ...
            if (options.multipleAdd) {
                ...
                _medit.multiItem.expandable($item.find('tr').data('multi-edit-data'), options.itemActions, multiRule).appendTo($item);
                // Expandable icon/action
                $item.find('td:first').prepend($('<div>').addClass('expand').click(function () {
                ...
                buttons: [{
                        ...
                        click: function () {
                            ...
                            $dataList.fadeOut(function () {
                                complete($.map($listView.find('tr.multi-edit-selected'), 
        ...
        refreshItemWidths: function ($multi) {
            $multi.find('.data-body').width($multi.find('form > table.multi-edit').width());
        ...
        loadingItem: function ($multi, label) {
            ...
            // Set label
            if (label) {
                $loading.append($('<div>').addClass('label').append($('<span>').html(_l(label))));
            }
            return $loading;
        },
        ...
        multiItem: {
            ...
            itemRow: function (item, itemActions, multiRule, $tbody) {
                ...
                $tr.append($('<td>').addClass('state').appendTo($tr).append($('<span>').text(item._itemStateLabel ? _l(item._itemStateLabel) + ' - ' + itemState :
                                ...
                                response: {
                                    success: function (args) {
                                        if (itemActionID == 'destroy') {
                                            ...
                                            cloudStack.ui.notifications.add(notification, success, successArgs, error, errorArgs);
                                        }
                                    },
                                    error: function (message) {
                                        if (message) {
    ...
    $.fn.multiEdit = function (args) {
        ...
        var $thead = $('<tr>').appendTo($('<thead>').appendTo($inputTable));
        if (!args.doNotShowInputTable) {
            var $inputForm = $('<tr>').appendTo($('<tbody>').appendTo($inputTable));
        }
            ...
            if (field.select) {
                ...
                if (field.range) {
                    ...
                    $(field.range).each(function () {
                        //e.g. field.range = ['privateport', 'privateendport'];
                        var $input = $('<input>')
                            ...
                            .appendTo($('<div>').addClass('range-item').appendTo($range));
                        if (field.isDisabled)
                            $input.hide();
                ...
                $('<div>').addClass('button add-vm custom-action')
                    ...
                    .click(function () {
                    if (field.custom.requireValidation && !$multiForm.valid())
                        return false;
                ...
                $addVM = $('<div>').addClass('button add-vm').html(_l('label.add')).appendTo($td);
            }
            if (field.desc) {
        ...
        if ($addVM) {
            $addVM.bind('click', function () {
                ...
                _medit.vmList($multi, args.listView, args.context, multipleAdd, _l('label.add.vms'), addItem);
                return true;
            });
        }
                ...
                response: {
                    success: function (args) {
                        ...
                        $(args.data).each(function () {
                            ...
                            _medit.addItem(data, fields, $multi, itemData, actions, {
