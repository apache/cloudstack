    ...
    var notifications = {
        ...
        cornerAlert: function (args, options) {
            ...
            var $cornerAlert = $('<div>').addClass('notification corner-alert')
                .hide()
                .appendTo('html body')
                .append($('<div>').addClass('top-arrow'), $('<div>').addClass('title').append($('<span>').html(options.error ? options.error : _l('label.task.completed'))))
                .append($('<div>').addClass('message')
                .append($('<span>').html(_l(args.message))));
            if (options.error) {
            ...
            var pollTimer = setInterval(function () {
                args.poll({
                    ...
                    error: function (args) {
                        ...
                        $item.removeClass('pending').addClass('error').append($('<div>').addClass('subtitle').html(args && args.message ?
        ...
        removeItem: function ($popup, $item) {
            if ($item.closest('li').hasClass('pending'))
                return false;
            $item.remove();
            return true;
        },
        ...
        popup: {
            create: function ($attachTo) {
                ...
                $('<div>').addClass('button clear-list')
                    .append($('<span>').html(_l('label.clear.list'))))
                    .append($('<div>').addClass('button close')
                    .append($('<span>').html(_l('label.close')))))
                    .css({
                    position: 'absolute'
                })
                    .data('notifications-attach-to', $attachTo)
                    .hide();
                if (!$attachTo.hasClass('notifications'))
                    $attachTo.addClass('notifications');
