...
public class LibvirtCleanupKbossValidationCommandWrapper extends CommandWrapper<CleanupKbossValidationCommand, Answer, LibvirtComputingResource> {
     ...
     * Since the <code>getStoragePoolByURI</code> and <code>deleteStoragePool</code> have a reference counter, where the first method increases the count and the second one
     * decreases the count, we must call the deleteStoragePool twice so that the command is count negative.
     * */
