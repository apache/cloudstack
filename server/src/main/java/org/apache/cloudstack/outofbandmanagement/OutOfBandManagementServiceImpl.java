...
public class OutOfBandManagementServiceImpl extends ManagerBase implements OutOfBandManagementService, Manager, Configurable {
        ...
        try {
            ...
            if (sentCount != null && sentCount <= 0) {
                ...
                if (concurrentUpdateResult) {
                    final String subject = String.format("Out-of-band management auth-error detected for host %s in cluster [id: %d] and zone [id: %d].", host, host.getClusterId(), host.getDataCenterId());
    ...
    public OutOfBandManagementResponse configure(final Host host, final ImmutableMap<OutOfBandManagement.Option, String> options) {
        ...
        String eventDetails = String.format("host [id:%d, name: %s], configuration: %s:%s", host.getId(), host.getName(), outOfBandManagementConfig.getAddress(), outOfBandManagementConfig.getPort());
    ...
    public OutOfBandManagementResponse executePowerOperation(final Host host, final OutOfBandManagement.PowerOperation powerOperation, final Long timeout) {
        ...
        if (driverResponse == null) {
            throw new CloudRuntimeException(String.format("Out-of-band Management action (%s) on host (%s) failed due to no response from the driver", powerOperation, host));
        }
        ...
        if (!driverResponse.isSuccess()) {
            String errorMessage = String.format("Out-of-band Management action (%s) on host (%s) failed with error: %s", powerOperation, host, driverResponse.getError());
            if (driverResponse.hasAuthFailure()) {
                errorMessage = String.format("Out-of-band Management action (%s) on host (%s) failed due to authentication error: %s. Please check configured credentials.", powerOperation, host, driverResponse.getError());
    ...
    public OutOfBandManagementResponse changePassword(final Host host, final String newPassword) {
        ...
        if (Strings.isNullOrEmpty(newPassword)) {
            throw new CloudRuntimeException(String.format("Cannot change out-of-band management password as provided new-password is null or empty for host %s.", host));
        }
        ...
        if (!(options.containsKey(OutOfBandManagement.Option.PASSWORD) && !Strings.isNullOrEmpty(options.get(OutOfBandManagement.Option.PASSWORD)))) {
            throw new CloudRuntimeException(String.format("Cannot change out-of-band management password as we've no previously configured password for host %s.", host));
        }
        final OutOfBandManagementDriver driver = getDriver(outOfBandManagementConfig);
        ...
        final boolean changePasswordResult = Transaction.execute(new TransactionCallback<Boolean>() {
            ...
            public Boolean doInTransaction(TransactionStatus status) {
                ...
                try {
                    ...
                    throw new CloudRuntimeException(String.format("Failed to change out-of-band management password for host (%s) due to driver error: %s", host, e.getMessage()));
                }

                if (!driverResponse.isSuccess()) {
                    throw new CloudRuntimeException(String.format("Failed to change out-of-band management password for host (%s) with error: %s", host, driverResponse.getError()));
                }
                    ...
                    if (isOutOfBandManagementEnabled(host)) {
                        ...
                        if (transitionPowerStateToDisabled(Collections.singletonList(host))) {
                            if (LOG.isDebugEnabled()) {
                                LOG.debug("Out-of-band management was disabled in zone/cluster/host, disabled power state for host: " + host);
                            }
                        }
                    }
                }
