...
(function ($, cloudStack) {
    ...
    var _breadcrumb = cloudStack.ui.widgets.browser.breadcrumb = {
        ...
        create: function ($panel, title) {
            ...
            return cloudStack.ui.event.elem('cloudBrowser', 'breadcrumb', $('<div>')
                .append($('<li>')
                .attr({
                title: title
            })
        ...
        filter: function ($panels) {
            ...
            $panels.each(function () {
                var $panel = $(this);
                $.merge($result, $.merge($breadcrumbs.filter(function () {
    ...
    var _panel = cloudStack.ui.widgets.browser.panel = {
        ...
        topIndex: function ($container) {
            ...
            return Math.max.apply(null, $.map($container.find('div.panel'), function (elem) {
        ...
        create: function ($container, options) {
            ...
            // Shadow
            $('<div>').addClass('shadow')).append(options.data);
            return $panel;
        }
    };
    /**
    ...
    $.widget('cloudStack.cloudBrowser', {
        ...
        selectPanel: function (args) {
            ...
            _breadcrumb.filter($('div.panel.maximized')
                .removeClass('maximized')
                .addClass('reduced')).removeClass('active maximized');
            $toRemove.remove();
            $toShow.show();
            $panel.css({
        ...
        addPanel: function (args) {
            ...
            // Panel initial state
            if ($panel.index() == 0)
                $panel.addClass('always-maximized');
            $panel.css(_panel.initialState($container, $panel));
            // Panel slide-in
            ...
            if (!$panel.index()) {
                ...
                if (args.complete)
                    args.complete($panel, _breadcrumb.filter($panel));
            }
            else {
                // Animate slide-in
                $panel.css({
                ...
                if ($panel.is(':visible') && args.complete)
                    args.complete($panel);
            }
            ;
            return $panel;
    ...
    $(document).on('click', '#breadcrumbs li', cloudStack.ui.event.bind('cloudBrowser', {
