...
-- vm_id=0 is the rule's "own" row (a VM-scoped rule's single target, or a group-scoped rule's
-- all-members aggregate); a group-scoped rule inherited by its members additionally gets one row
...
-- Per-boot-group override of the global readiness ConfigKeys (timeout/max-reboot-attempts); the
-- 'name' values are the exact ConfigKey key strings, so override resolution needs no key-mapping
