    ...
    <a-table
      ...
      <template #Buttons="{ record }">
        <tooltip-button
          ...
          :disabled="!('enableVMSchedule' in $store.getters.apis)"
          :title="$t('enable.schedule')"
          icon="play-circle-outlined"
          @onClick="enableVMSchedule(record)" />
        <tooltip-button
          v-if="record.state==='ENABLED'"
          style="margin-right: 5px"
          :disabled="!('disableVMSchedule' in $store.getters.apis)"
          :title="$t('disable.schedule')"
          icon="pause-circle-outlined"
          @onClick="disableVMSchedule(record)" />
        <tooltip-button
          style="margin-right: 5px"
...
export default {
  ...
  methods: {
    ...
    onShowDeleteModal (record) {
      const params = {}
      params.vmscheduleid = record.id
      api('deleteVMSchedule', params).then(json => {
        this.$notification.success({
          message: this.$t('label.delete.schedule'),
          description: this.$t('message.success.delete.vm.schedule')
        })
      }).catch(error => {
    ...
    enableVMSchedule (record) {
      const params = {}
      params.vmscheduleid = record.id
      api('enableVMSchedule', params).then(json => {
        this.$notification.success({
          message: this.$t('label.enable.schedule'),
          description: this.$t('message.success.enable.vm.schedule')
        })
      }).catch(error => {
        this.$notifyError(error)
      }).finally(() => {
    ...
    disableVMSchedule (record) {
      const params = {}
      params.vmscheduleid = record.id
      api('disableVMSchedule', params).then(json => {
        this.$notification.success({
          message: this.$t('label.disable.schedule'),
          description: this.$t('message.success.disable.vm.schedule')
        })
      }).catch(error => {
        this.$notifyError(error)
      }).finally(() => {
