...
class CsQuagga(CsDataBag):
    ...
    def process(self):
        ...
        changed = False
        #process zebra
        changed=self.applyConf(ZEBRA_CONF_NEW, zebra_config, ZEBRA_CONF)

        #process ospfd
        changed=self.applyConf(OSPFD_CONF_NEW, zebra_config, OSPFD_CONF)
    ...
    def applyConf(self, confNew, config, confOld):
        #process zebra
        conf_new = CsFile(confNew)
        conf_new.empty()
        changed = False
        for x in config:
            [conf_new.append(w, -1) for w in x.split('\n')]
        conf_new.commit()
        
        conf_old = CsFile(confOld)
        if not conf_old.compare(conf_new):
            CsHelper.copy(confNew, confOld)
            changed = True
