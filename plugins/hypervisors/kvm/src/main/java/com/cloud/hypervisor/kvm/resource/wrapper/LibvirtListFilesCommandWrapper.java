...
import org.apache.cloudstack.backup.ListFilesCommand;
...
@ResourceWrapper(handles = ListFilesCommand.class)
public class LibvirtListFilesCommandWrapper extends CommandWrapper<ListFilesCommand, Answer, LibvirtComputingResource> {
    @Override
    public Answer execute(ListFilesCommand cmd, LibvirtComputingResource serverResource) {
        ...
        try {
            ...
            String fullPath = imagePool.getLocalPathFor(cmd.getBackupPath());
            try (GuestfishClient guestfishClient = new GuestfishClient(fullPath, cmd.getVolumeId())) {
                return guestfishClient.listFiles(cmd.getFilesystem(), cmd.getDirPath(), cmd.isSymlink());
            }
        } finally {
