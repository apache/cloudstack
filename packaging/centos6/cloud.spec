...
if [ "%{_servletengine}" == "jetty" ] ; then
   echo "Building with Jetty"
   FLAGS="$FLAGS -Djetty"
else
   echo "Building with Tomcat"
   FLAGS="$FLAGS -Dcatalina"
fi
...
cp packaging/common/cloudstack-sccs ${RPM_BUILD_ROOT}/usr/bin
...
echo "%{_servletengine}" > ${RPM_BUILD_ROOT}%{_sysconfdir}/%{name}/management/servlet-engine
/bin/touch ${RPM_BUILD_ROOT}%{_localstatedir}/log/%{name}/management/servlet-engine.out
...
cp -r client/%{_servletengine}/target/cloudstack-%{_servletengine}-standalone/* ${RPM_BUILD_ROOT}%{_datadir}/%{name}-management/
...
cp setup/db/*.* ${RPM_BUILD_ROOT}%{_datadir}/%{name}-management/setup
...
install -D packaging/common/cloud-agent.rc ${RPM_BUILD_ROOT}%{_sysconfdir}/init.d/%{name}-agent
...
%post usage
touch %{_localstatedir}/run/%{name}-usage.pid
chown cloud. %{_localstatedir}/run/%{name}-usage.pid
...
%attr(0644,cloud,cloud) %{_localstatedir}/log/%{name}/management/servlet-engine.out
...
%changelog
* Thu Jun 4 2015 Rafael da Fonseca <rsafonseca@gmail.com> 4.6.0
- Big package refurbishment
