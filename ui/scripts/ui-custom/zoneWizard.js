...
(function ($, cloudStack) {
    ...
    var getData = function ($wizard, options) {
        ...
        var $physicalNetworkItems = $wizard.find('.steps .setup-physical-network .select-container.multi').filter(':not(.disabled)');
        var $publicTrafficItems = $wizard.find('.steps .setup-public-traffic .data-body .data-item');
        var $storageTrafficItems = $wizard.find('.steps .setup-storage-traffic .data-body .data-item');
        var groupedForms = {};
        ...
        groupedForms.physicalNetworks = $.map($physicalNetworkItems, function (network) {
            var $network = $(network);
            ...
            return {
                ...
                // Traffic type list
                trafficTypes: $.map($network.find('.traffic-type-draggable'), function (trafficType) {
        ...
        groupedForms.publicTraffic = $.map($publicTrafficItems, function (publicTrafficItem) {
        ...
        groupedForms.storageTraffic = $.map($storageTrafficItems, function (storageTrafficItem) {
    ...
    var physicalNetwork = {
        ...
        getTrafficType: function (trafficTypeID, $container) {
            ...
            if (physicalNetwork.isTrafficTypeClone($trafficType) && !$container.closest('.select-container.multi').length) {
                ...
                return $trafficType.filter(function () {
                    return $(this).closest(physicalNetwork.getOriginalTrafficContainer($trafficType)).length;
                });
            }
            return $trafficType;
        },
        ...
        assignTrafficType: function (trafficTypeID, $physicalNetwork, data) {
            ...
            if ($physicalNetwork.find('.traffic-type-draggable[traffic-type-id=' + trafficTypeID + ']').length)
                return false;
        ...
        unassignTrafficType: function ($trafficType) {
            ...
            if (!physicalNetwork.isTrafficTypeClone($trafficType) &&
                ...
                physicalNetwork.assignTrafficType(trafficTypeID, $physicalNetworks.filter(':first'));
                if (physicalNetwork.isTrafficTypeClone($trafficType) &&
        ...
        add: function ($wizard) {
            ...
            var $nameField = $('<div>').addClass('field name').append($('<div>').addClass('name').append($('<label>').html(_l('label.physical.network.name'))), $('<div>').addClass('value').append($('<input>').attr({
                type: 'text'
            ...
            if ($wizard.find('.select-network-model input:radio[name=network-model]:checked').val() == 'Advanced') {
                $nameField.append($('<div>').append($('<span style=\"font-size:11px;padding-right:5px;padding-left:40px\">').html(_l('label.isolation.method')), $('<select>').append($('<option>').attr({
                    value: 'VLAN'
                }).html('VLAN'), $('<option>').attr({
                    value: 'GRE'
                }).html('GRE'), $('<option>').attr({
                    value: 'STT'
                }).html('STT'), $('<option>').attr({
                    value: 'BCF_SEGMENT'
                }).html('BCF_SEGMENT'), $('<option>').attr({
                    value: 'SSP'
                }).html('SSP'), $('<option>').attr({
                    value: 'VXLAN'
                }).html('VXLAN'), $('<option>').attr({
                    value: 'ODL'
                }).html('ODL'), $('<option>').attr({
                    value: 'L3VPN'
                }).html('L3VPN'), $('<option>').attr({
                    value: 'VSP'
                }).html('VSP'), $('<option>').attr({
                    value: 'VCS'
                }).html('VCS'))));
            }
            ...
            var $dropContainer = $('<div>').addClass('drop-container').append($('<span>').addClass('empty-message').html('Drag and drop traffic types you would like to add here.'), $('<ul>').hide()).droppable({
            ...
            $physicalNetworkItem.append($icon, $nameField, $idField, $dropContainer);
    ...
    cloudStack.uiCustom.zoneWizard = function (args) {
        return function () {
            ...
            var completeAction = function () {
                ...
                var makeMessage = function (message, isError) {
                    var $li = $('<li>')
                        ...
                        .append($('<span>').addClass('icon').html('&nbsp;'), $('<span>').addClass('text').html(message));
                ...
                args.action({
                    ...
                    uiSteps: $.map($wizard.find('.steps > div'), function (step) {
            ...
            var showStep = function (index, goBack, options) {
                ...
                var stepPreFilter = cloudStack.zoneWizard.preFilters[$targetStep.attr('zone-wizard-prefilter')];
                // Bypass step check
                if (stepPreFilter && !stepPreFilter({
                    data: formState,
                    groupedData: groupedFormState
                })) {
                    return showStep(!goBack ? index + 1 : index - 1, goBack);
                }
                if (formID) {
                ...
                if ($uiCustom.length) {
                    $uiCustom.each(function () {
                        ...
                        $item.replaceWith(args.customUI[id]({
                            data: formState,
            ...
            $wizard.find('select').change(function (event) {
                ...
                if ($target.is('select.ip-scope')) {
                    $accountSpecific.hide();
                    if ($target.val() == 'account-specific')
