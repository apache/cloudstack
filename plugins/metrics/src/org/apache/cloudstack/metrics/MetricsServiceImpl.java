...
public class MetricsServiceImpl extends ComponentLifecycleBase implements MetricsService {
    ...
    public InfrastructureResponse listInfrastructure() {
        ...
        response.setClusters(clusterDao.listAllCusters(null).size());
        response.setHosts(hostDao.listAllHostsByType(Host.Type.Routing).size());
        ...
        int cpuSockets = 0;
        for (final Host host : hostDao.listAllHostsByType(Host.Type.Routing)) {
