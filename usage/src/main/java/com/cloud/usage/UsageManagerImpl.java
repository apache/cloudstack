...
public class UsageManagerImpl extends ManagerBase implements UsageManager, Runnable {
        ...
        if (EventTypes.EVENT_VM_START.equals(event.getType())) {
            ...
            if (usageInstances != null) {
                if (usageInstances.size() > 1) {
                    logger.warn("Found multiple entries for an Instance running with id: " + vmId + ", ending them all...");
                }
                for (UsageVMInstanceVO usageInstance : usageInstances) {
                    usageInstance.setEndDate(event.getCreateDate());
    ...
    private void createVmDiskHelperEntry(VmDiskStatisticsVO vmDiskStat, UsageVmDiskVO usageVmDiskStat, long timestamp) {
        ...
        if (bytesRead < 0) {
            logger.warn("Calculated negative value for bytes read: " + toHumanReadableSize(bytesRead) + ", Instance disk stats say: " + toHumanReadableSize(vmDiskStat.getAggBytesRead()) +
                    ", previous Instance disk usage was: " + toHumanReadableSize(currentAccountedBytesRead));
            bytesRead = 0;
        }
        if (bytesWrite < 0) {
            logger.warn("Calculated negative value for bytes write: " + toHumanReadableSize(bytesWrite) + ", Instance disk stats say: " + toHumanReadableSize(vmDiskStat.getAggBytesWrite()) +
                    ", previous Instance disk usage was: " + toHumanReadableSize(currentAccountedBytesWrite));
            bytesWrite = 0;
        }
    ...
    protected void deleteUsageVMSnapshot(UsageEventVO event) {
        ...
        if (usageVMSnapshots.size() > 1) {
            logger.warn(String.format("More than one usage entry for Instance Snapshot for Instance id [%s] assigned to Account [%s] domain [%s] and zone [%s]; marking them all as deleted.", vmId,
                    accountId, domainId, zoneId));
        }
