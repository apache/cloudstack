 ...
 */
public class Link {
    private static final Logger LOG = LogFactory.getLogger(Link.class);
        ...
        while (processedLen < totalLen) {
            ...
            while (headRemaining > 0) {
                if (LOG.isTraceEnabled()) {
                    LOG.trace("Writing Header " + headRemaining);
                }
            ...
            while (dataRemaining > 0) {
                if (LOG.isTraceEnabled()) {
                    LOG.trace("Writing Data " + dataRemaining);
                }
    ...
    public byte[] read(SocketChannel ch) throws IOException {
        if (_readHeader) {   // Start of a packet
            ...
            if (_readBuffer.hasRemaining()) {
                LOG.trace("Need to read the rest of the packet length");
                return null;
            }
            _readBuffer.flip();
            ...
            if (LOG.isTraceEnabled()) {
                LOG.trace("Packet length is " + readSize);
            }
            ...
            if (_readBuffer.capacity() < readSize) {
                if (LOG.isTraceEnabled()) {
                    LOG.trace("Resizing the byte buffer from " + _readBuffer.capacity());
                }
        ...
        if (_readBuffer.hasRemaining()) {   // We're not done yet.
            if (LOG.isTraceEnabled()) {
                LOG.trace("Still has " + _readBuffer.remaining());
            }
            return null;
        }
        ...
        while (_readBuffer.hasRemaining()) {
            ...
            if (LOG.isTraceEnabled()) {
                LOG.trace("Done with packet: " + appBuf.limit());
            }
        }
    ...
    public void send(ByteBuffer[] data, boolean close) throws ClosedChannelException {
        ...
        if (LOG.isTraceEnabled()) {
            LOG.trace("Sending packet of length " + remaining);
        }
    ...
    public boolean write(SocketChannel ch) throws IOException {
        ...
        while ((data = _writeQueue.poll()) != null) {
            if (data.length == 0) {
                if (LOG.isTraceEnabled()) {
                    LOG.trace("Closing connection requested");
                }
                return true;
            }
    ...
    public static SSLEngine initServerSSLEngine(final CAService caService, final String clientAddress) throws GeneralSecurityException, IOException {
        ...
        LOG.error("CA service is not configured, by-passing CA manager to create SSL engine");
    ...
    public static SSLContext initClientSSLContext() throws GeneralSecurityException, IOException {
        ...
        if (stream != null) {
            ...
            LOG.warn("Failed to load keystore, using trust all manager");
        }
    ...
    private static HandshakeHolder doHandshakeUnwrap(final SocketChannel socketChannel, final SSLEngine sslEngine,
        ...
        if (socketChannel.read(peerNetData) < 0) {
            ...
            try {
                ...
                LOG.warn("This SSL engine was forced to close inbound due to end of stream.");
            }
        ...
        try {
            ...
            LOG.error(String.format("SSL error caught during unwrap data: %s, for local address=%s, remote address=%s. The client may have invalid ca-certificates.",
    ...
    private static HandshakeHolder doHandshakeWrap(final SocketChannel socketChannel, final SSLEngine sslEngine,
        ...
        try {
            ...
            LOG.error(String.format("SSL error caught during wrap data: %s, for local address=%s, remote address=%s.",
                    sslException.getMessage(), socketChannel.getLocalAddress(), socketChannel.getRemoteAddress()));
            sslEngine.closeOutbound();
    ...
    public static boolean doHandshake(final SocketChannel socketChannel, final SSLEngine sslEngine) throws IOException {
        ...
        while (handshakeStatus != SSLEngineResult.HandshakeStatus.FINISHED
            ...
            if (timeTaken > 15000L) {
                LOG.warn("SSL Handshake has taken more than 15s to connect to: " + socketChannel.getRemoteAddress() +
            ...
            switch (handshakeStatus) {
                ...
                case NEED_TASK:
                    ...
                    while ((task = sslEngine.getDelegatedTask()) != null) {
                        if (LOG.isTraceEnabled()) {
                            LOG.trace("SSL: Running delegated task!");
                        }
