...
fix_hostname() {
  # 0Fix hostname in openssh-server generated keys
...
#TODO check this location for vhd-util. This build uses this version of vhd-util as is and is not built from scratch
#nor is a SHA (or some other hash) checked for validity. Utility is made part of the system vm image without checking
#
fix_vhdutil() {
