...
public class AgentManagerImpl implements AgentManager, HandlerFactory, ResourceService, Manager {
    ...
    public boolean deleteHost(long hostId) {
        ...
        try {
            ...
            // For pool ids you got, delete entries in pool table where type is local storage
    ...
    protected AgentAttache createAttache(long id, HostVO server, Link link) {
        ...
        final AgentAttache attache = new ConnectedAgentAttache(id, link, server.getStatus() == Status.Maintenance || server.getStatus() == Status.ErrorInMaintenance
    ...
    protected AgentAttache createAttache(long id, HostVO server, ServerResource resource) {
        ...
        final DirectAgentAttache attache = new DirectAgentAttache(id, resource, server.getStatus() == Status.Maintenance || server.getStatus() == Status.ErrorInMaintenance
    ...
    private void createCapacityEntry(final StartupCommand startup, HostVO server) {
        ...
        if (startup instanceof StartupStorageCommand) {
            ...
            if (ssCmd.getResourceType() == Storage.StorageResourceType.STORAGE_HOST) {
                CapacityVO capacity = new CapacityVO(server.getId(), server.getDataCenterId(), server.getPodId(), server.getClusterId(), 0L, server.getTotalSize() * _overProvisioningFactor,
