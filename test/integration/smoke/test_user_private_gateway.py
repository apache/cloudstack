...
class TestUserPrivateGateways(cloudstackTestCase):
    ...
    def test_01_create_private_gateway_with_vlan(self):
        ...
        # Create VPC
        vpc1 = self.create_vpc_for_account(self.admin_apiclient, None, None, None)
        vpc2 = self.create_vpc_for_account(self.domainadmin_apiclient, None, None, None)
        vpc3 = self.create_vpc_for_account(self.normaluser_apiclient, None, None, None)
        vpc4 = self.create_vpc_for_account(self.domainadmin_apiclient, None, None, self.project)

        # Create VPC private gateway with vlan (can only be done by ROOT admin)
        private_gateway1 = self.create_vpc_private_gateway(self.admin_apiclient, vpc1, 10, None, True)
        self.create_vpc_private_gateway(self.domainadmin_apiclient, vpc2, 11, None, False)
        self.create_vpc_private_gateway(self.normaluser_apiclient, vpc3, 12, None, False)
        self.create_vpc_private_gateway(self.domainadmin_apiclient, vpc4, 13, None, False)
        private_gateway2 = self.create_vpc_private_gateway(self.admin_apiclient, vpc2, 11, None, True)
        private_gateway3 = self.create_vpc_private_gateway(self.admin_apiclient, vpc3, 12, None, True)
        private_gateway4 = self.create_vpc_private_gateway(self.admin_apiclient, vpc4, 13, None, True)

        # Delete VPC private gateway (should succeed by ROOT admin)
        self.delete_vpc_private_gateway(self.admin_apiclient, private_gateway1, True)
        self.delete_vpc_private_gateway(self.domainadmin_apiclient, private_gateway2, False)
        self.delete_vpc_private_gateway(self.normaluser_apiclient, private_gateway2, False)
        self.delete_vpc_private_gateway(self.domainadmin_apiclient, private_gateway4, False)
        self.delete_vpc_private_gateway(self.admin_apiclient, private_gateway2, True)
        self.delete_vpc_private_gateway(self.admin_apiclient, private_gateway3, True)
        self.delete_vpc_private_gateway(self.admin_apiclient, private_gateway4, True)

        # Delete VPC
        self.delete_vpc(self.admin_apiclient, vpc1)
        self.delete_vpc(self.domainadmin_apiclient, vpc2)
        self.delete_vpc(self.normaluser_apiclient, vpc3)
        self.delete_vpc(self.domainadmin_apiclient, vpc4)
    ...
    def test_02_create_private_gateway_with_associated_network(self):
        ...
        isolated_network1 = self.create_isolated_network_for_account(self.admin_apiclient, None, None, None)
        isolated_network2 = self.create_isolated_network_for_account(self.admin_apiclient, self.sub_domain, self.domain_admin, None)
        isolated_network3 = self.create_isolated_network_for_account(self.admin_apiclient, self.sub_domain, self.normal_user, None)
        isolated_network4 = self.create_isolated_network_for_account(self.admin_apiclient, self.sub_domain, None, self.project)

        # Check state of isolated networks (should be Allocated)
        self.check_network_state(self.admin_apiclient, isolated_network1, None, NETWORK_STATE_ALLOCATED)
        self.check_network_state(self.domainadmin_apiclient, isolated_network2, None, NETWORK_STATE_ALLOCATED)
        self.check_network_state(self.normaluser_apiclient, isolated_network3, None, NETWORK_STATE_ALLOCATED)
        self.check_network_state(self.domainadmin_apiclient, isolated_network4, self.project, NETWORK_STATE_ALLOCATED)
        ...
        # Create VPC tier
        vpc1_tier1 = self.create_vpc_tier_for_account(self.admin_apiclient, vpc1)
        vpc2_tier1 = self.create_vpc_tier_for_account(self.domainadmin_apiclient, vpc2)
        vpc3_tier1 = self.create_vpc_tier_for_account(self.normaluser_apiclient, vpc3)
        vpc4_tier1 = self.create_vpc_tier_for_account(self.domainadmin_apiclient, vpc4, self.project)

        # Create VPC private gateway with associated network (can be done by ROOT admin / domain admin / normal user)
        private_gateway1 = self.create_vpc_private_gateway(self.admin_apiclient, vpc1, None, isolated_network1, True)
        private_gateway2 = self.create_vpc_private_gateway(self.domainadmin_apiclient, vpc2, None, isolated_network2, True)
        private_gateway3 = self.create_vpc_private_gateway(self.normaluser_apiclient, vpc3, None, isolated_network3, True)
        private_gateway4 = self.create_vpc_private_gateway(self.domainadmin_apiclient, vpc4, None, isolated_network4, True)

        # Check state of isolated networks (should be Implemented)
        self.check_network_state(self.admin_apiclient, isolated_network1, None, NETWORK_STATE_IMPLEMENTED)
        self.check_network_state(self.domainadmin_apiclient, isolated_network2, None, NETWORK_STATE_IMPLEMENTED)
        self.check_network_state(self.normaluser_apiclient, isolated_network3, None, NETWORK_STATE_IMPLEMENTED)
        self.check_network_state(self.domainadmin_apiclient, isolated_network4, self.project, NETWORK_STATE_IMPLEMENTED)

        # Delete VPC private gateway of domain admin (should succeed)
        self.delete_vpc_private_gateway(self.domainadmin_apiclient, private_gateway2, True)

        # Wait for network GC to shut down the isolated networks
        gc_wait = Configurations.list(self.admin_apiclient, name="network.gc.wait")
        gc_interval = Configurations.list(self.admin_apiclient, name="network.gc.interval")
        total_sleep = 360
        ...
        self.delete_network(vpc1_tier1, self.admin_apiclient, True)
        self.delete_network(vpc2_tier1, self.domainadmin_apiclient, True)
        self.delete_network(vpc3_tier1, self.normaluser_apiclient, True)
        self.delete_network(vpc4_tier1, self.domainadmin_apiclient, True)
        ...
        self.delete_network(isolated_network1, self.admin_apiclient, True)
        self.delete_network(isolated_network2, self.domainadmin_apiclient, True)
        self.delete_network(isolated_network3, self.normaluser_apiclient, True)
        self.delete_network(isolated_network4, self.domainadmin_apiclient, True)
