...
public abstract class AbstractOIDCOAuth2Provider extends AdapterBase implements UserOAuth2Authenticator {
    ...
    protected AbstractOIDCOAuth2Provider() {
        this(HttpClientBuilder.create().build());
    }

    protected AbstractOIDCOAuth2Provider(CloseableHttpClient httpClient) {
        ...
        if (StringUtils.isBlank(idToken)) {
            ...
            try (CloseableHttpResponse response = httpClient.execute(post)) {
                ...
                if (response.getStatusLine().getStatusCode() != 200) {
                    throw new CloudRuntimeException(String.format("%s error during token generation: %s", getName(), body));
                }
                ...
                throw new CloudRuntimeException(String.format("Unable to connect to %s server", getName()), e);
            }
        }
