...
public class VgpuProfileDaoImpl extends GenericDaoBase<VgpuProfileVO, Long> implements VgpuProfileDao {
    ...
    @Override
    public List<VgpuProfileVO> findByNameAndCardIds(String name, List<Long> cardIdList) {
        SearchCriteria<VgpuProfileVO> sc = allFieldSearch.create();
        sc.setParameters("name", name);
        sc.setParameters("cardId", cardIdList.toArray());
        return listBy(sc);
    }

    @Override
    public Pair<List<VgpuProfileVO>, Integer> searchAndCountVgpuProfiles(
            Long id, String name, String keyword, Long gpuCardId, Long startIndex, Long pageSize
    ) {
