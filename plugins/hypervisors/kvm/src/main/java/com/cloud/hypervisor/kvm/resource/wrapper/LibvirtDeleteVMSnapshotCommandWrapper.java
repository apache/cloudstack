    ...
    public Answer execute(final DeleteVMSnapshotCommand cmd, final LibvirtComputingResource libvirtComputingResource) {
        ...
        try {
            ...
            if (dm == null) {
                ...
                logger.debug("Can not find Instance Snapshot " + cmd.getTarget().getSnapshotName() + " on Instance: " + vmName + ", return true");
                return new DeleteVMSnapshotAnswer(cmd, cmd.getVolumeTOs());
            } else if (tryingResume) {
                logger.error("Failed to resume Instance after delete snapshot " + cmd.getTarget().getSnapshotName() + " on Instance: " + vmName + " return true : " + e);
                return new DeleteVMSnapshotAnswer(cmd, cmd.getVolumeTOs());
            }
