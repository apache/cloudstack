...
class TestBrowseUploadVolume(cloudstackTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.uploadurl = cls.testdata["browser_upload_volume"][
            cls.uploadvolumeformat]["url"]
        cls.volname = cls.testdata["browser_upload_volume"][
            cls.uploadvolumeformat]["diskname"]
        cls.md5sum = cls.testdata["browser_upload_volume"][
            cls.uploadvolumeformat]["checksum"]
        cls.zone = get_zone(cls.apiclient, cls.testClient.getZoneForTests())
        cls.domain = get_domain(cls.apiclient)
        cls.pod = get_pod(cls.apiclient, cls.zone.id)

        if cls.uploadvolumeformat == "QCOW2" or cls.uploadvolumeformat == "VHD":
            cls.extuploadurl = cls.testdata["browser_upload_volume_extended"][
                cls.uploadvolumeformat]["url"]
    ...
    def tready(self):
        if self.template == FAILED:
            self.skipTest("Check for default cent OS template readiness ")
        return
    ...
    def validate_uploaded_volume(self, up_volid, volumestate):
        ...
        if volumestate == "UploadAbandoned":
            time.sleep((uploadtimeout * 60) + monitoringinterval)
        else:
            time.sleep(uploadtimeout + monitoringinterval)
    ...
    def browse_upload_volume_with_out_zoneid(self):
        ...
        success = False
        try:
            getuploadparamsresponce = self.apiclient.getUploadParamsForVolume(
                cmd)
        except Exception as ex:
            if "postuploadvolume failed" in str(ex):
                success = True
        self.assertEqual(
            success,
            True,
    ...
    def browse_upload_volume_onetime_url(self):
        cmd = getUploadParamsForVolume.getUploadParamsForVolumeCmd()
        cmd.zoneid = self.zone.id
        cmd.format = self.uploadvolumeformat
        cmd.name = self.volname + self.account.name + \
    ...
    def invalidupload(self):
        ...
        try:
            ...
            self.debug(ex)
            if "Image format: invalidformat is incorrect" in str(ex):
                success = True
        self.assertEqual(
            success,
            True,
    ...
    def invalidposturl(self):
        ...
        try:
            ...
            cmd.zoneid = self.zone.id
            cmd.format = self.uploadvolumeformat
            cmd.name = self.volname + self.account.name + "InvalidPostURL" +\
                (random.choice(string.ascii_uppercase))
            cmd.account = self.account.name
            cmd.domainid = self.domain.id
            getuploadparamsresponce = self.apiclient.getUploadParamsForVolume(
            ...
            signt = getuploadparamsresponce.signature
            posturl = "http://invalidposturl/2999834." + \
            ...
            if results.status_code == 200:
                self.fail(
                    "Upload is done with invalid post URL and This is not handled")
                return()

            self.validate_uploaded_volume(
                getuploadparamsresponce.id, 'UploadedAbandoned')
        ...
        self.assertEqual(
            success,
            True,
            "Upload works fine with Tampered Post URL. Failing")
    ...
    def reuse_url(self):
        ...
        self.validate_uploaded_volume(
            getuploadparamsresponce.id, 'UploadAbandoned')
        return("PASS")
    ...
    def validate_storage_cleanup(self, invalidpostvolume, cleanup_interval):
        ...
        if cleanup_interval >= ((uploadtimeout * 60) + monitorinterval):
            time.sleep(cleanup_interval + monitorinterval)
        else:
    ...
    def validate_max_vol_size(self, up_vol, volumestate):
        ...
        if (int(list_volume_response[0].size) /
                (1024 * 1024 * 1024)) > max_size:
            self.fail(
                "Global Config storage.max.volume.upload.size is not considered with Browser Based Upload volumes")
    ...
    def attach_deleted_volume(self, vmlist, volume):
        ...
        try:
            ...
            if "Please specify a volume with the valid type: DATADISK" in str(
                    ex):
                success = True
        self.assertEqual(
            success,
    ...
    def deletevolume_fail(self, volumeid):
        ...
        try:
            ...
            if "Please specify a volume that is not attached to any VM" in str(
                    ex):
                success = True
        self.assertEqual(
            success,
    ...
    def resize_fail(self, volumeid):
        ...
        try:
            ...
            if "Volume should be in ready or allocated state before attempting a resize" in str(
                    ex):
                success = True
        self.assertEqual(
            success,
    ...
    def resize_volume(self, volumeid):
        ...
        self.testdata["browser_upload_volume"][
            "browser_resized_disk_offering"]["disksize"] = 20
        ...
        while count < 3:
            ...
            for vol in list_volume_response:
                if vol.id == volumeid and int(vol.size) == (
    ...
    def destroy_vm(self, vmdetails):
        ...
        try:
            ...
            if "Unable to find a virtual machine with specified vmId" in str(
                    ex):
                success = True
    ...
    def uploadvol(self, getuploadparamsresponce):
        ...
        self.validate_uploaded_volume(
            getuploadparamsresponce.id, 'Uploaded')
    ...
    def invalidsignatureupload(self):
        ...
        success = False
        try:
            signt = "sdfsdfsfsdfsdffsgsdgsdgsd"
            posturl = getuploadparamsresponce.postURL
            metadata = getuploadparamsresponce.metadata
            expiredata = getuploadparamsresponce.expires
            url = self.uploadurl
            ...
            self.debug(results.status_code)
            if results.status_code == 200:
                self.fail("Upload is working with invalid signature")
                return()
        except Exception as ex:
            if "certificate verify failed" in str(ex):
                ...
                self.assertEqual(
                    success,
                    True,
                    "Upload  with invalid signature is handled appropriately")
        return()
    ...
    def test_01_Browser_volume_Life_cycle_tpath(self):
        ...
        try:
            ...
            self.debug(
                "========================= Test 1: Upload Browser based volume and validate ========================= ")
            browseup_vol = self.browse_upload_volume()

            self.debug(
                "========================= Test 2: Deploy a VM , Attach Uploaded Browser based volume and validate VM Operations========================= ")
            ...
            self.debug(
                "========================= Test 3: Restore VM with Uploaded volume attached========================= ")

            self.restore_vm(vm1details)

            self.debug(
                "========================= Test 4: Detach Uploaded volume and validation of VM operations after detach========================= ")
            ...
            self.debug(
                "========================= Test 5: Deploy New VM,Attach the detached Uploaded volume and validate VM operations after attach========================= ")
            ...
            self.debug(
                "========================= Test 6: Detach Uploaded volume and resize detached uploaded volume========================= ")
            ...
            self.debug(
                "========================= Test 7: Attach resized uploaded volume and validate VM operations========================= ")
            ...
            self.debug(
                "========================= Test 8: Try resizing uploaded state volume and validate the error scenario========================= ")
            ...
            self.debug(
                "========================= Test 9: Attach multiple uploaded volumes to a VM and validate VM operations========================= ")
            ...
            self.debug(
                "========================= Test 10:  Detach and delete uploaded volume========================= ")
            ...
            self.debug(
                "========================= Test 11:  Detach and download uploaded volume========================= ")
            ...
            self.debug(
                "========================= Test 12:  Delete detached uploaded volume========================= ")
            ...
            self.debug(
                "========================= Deletion of UnUsed VM's after test is complete========================= ")

            self.expunge_vm(vm2details)

            self.debug(
                "========================= Test 13:  Delete Uploaded State volume========================= ")
            ...
            self.debug(
                "========================= Test 14:  Destroy VM which has Uploaded volumes attached========================= ")
            ...
            self.debug(
                "========================= Test 15:  Recover destroyed VM which has Uploaded volumes attached========================= ")
            ...
            self.debug(
                "========================= Test 17:  Create Volume Backup Snapshot uploaded volume attached to the VM========================= ")
            ...
            self.debug(
                "========================= Test 18:  Create Volume from Backup Snapshot of attached uploaded volume========================= ")

            self.volume_snapshot_volume(snapshotdetails)

            self.debug(
                "========================= Test 19:  Create template from Backup Snapshot of attached uploaded volume========================= ")
            ...
            self.debug(
                "========================= Test 20: Upload Browser based volume with checksum and validate ========================= ")
            browseup_vol_withchecksum = self.browse_upload_volume_with_md5()

            self.debug(
                "========================= Test 21: Deploy a VM , Attach Uploaded Browser based volume with checksum and validate VM Operations========================= ")
            ...
            self.debug(
                "========================= Test 22: Detach Uploaded volume with checksum and validation of VM operations after detach========================= ")
    ...
    def test_02_SSVM_Life_Cycle_With_Browser_Volume_TPath(self):
        ...
        try:
            self.debug(
                "========================= Test 23: Stop and Start SSVM and Perform Browser based volume validations ========================= ")
            ...
            self.debug(
                "========================= Test 24: Reboot SSVM and Perform Browser based volume validations ========================= ")
            ...
            self.debug(
                "========================= Test 25: Reboot SSVM and Perform Browser based volume validations ========================= ")
    ...
    def test_03_Browser_Upload_Volume_Global_Config_TPath(self):
        ...
        try:
            ...
            self.debug(
                "========================= Test 26 Validate Storage.max.upload.size ========================= ")
    ...
    def test_04_Browser_Upload_Volume_Negative_Scenarios_TPath(self):
        ...
        try:
            self.debug(
                "========================= Test 27 Reuse the POST URL after expiry time========================= ")
            reuse_browse_up_vol = self.browse_upload_volume()
            result = self.reuse_url()
            if result == "FAIL":
                self.fail(
                    "Upload is not rejected when POST URL is reused after export time. Failing ")
            ...
            self.debug(
                "========================= Test 28 One Time POST URL before expiry time========================= ")
            one_browse_up_vol = self.browse_upload_volume_onetime_url()

            cmd = deleteVolume.deleteVolumeCmd()
            cmd.id = one_browse_up_vol.id
            self.apiclient.deleteVolume(cmd)

            self.debug(
                "========================= Test 29 upload with invalid checksum========================= ")
            invalidmd5upload = self.browse_upload_volume_with_invalid_md5()

            list_volume_response = Volume.list(
                self.apiclient,
                id=invalidmd5upload.id
            )
            ...
            if list_volume_response is None:
                ...
                cmd = deleteVolume.deleteVolumeCmd()
                cmd.id = invalidmd5upload.id
                self.apiclient.deleteVolume(cmd)

            self.debug(
                "========================= Test 30 Reboot SSVM after upload is completed=========================")
            ...
            self.debug(
                "========================= Test 31 Reboot SSVM after getting the upload volume params and before initiating the upload=========================")
            browse_up_vol = self.onlyupload()
            ...
            self.attach_deleted_volume(
                deletedvm1details, deleted_browse_up_vol)

            self.debug(
                "========================= Test 33 Upload Volume with Invalid Format=========================")
            self.invalidupload()

            self.debug(
                "========================= Test 34 Upload Mutliple Volumes with same display text=========================")
            samedisplaytext_browse_up_vol = self.browse_upload_volume()
            self.uploadwithsamedisplaytext(samedisplaytext_browse_up_vol)

            self.debug(
                "========================= Test 35 Upload Volume with custom offering id=========================")
    ...
    def test_05_Browser_Upload_Volume_MultiSSVM_Scenarios_TPath(self):
        ...
        self.tready()
        self.debug(
            "========================= Test 36 Upload volume with Multiple SSVM=========================")

        testresult = self.uploadvolwithmultissvm()
        if testresult == 0:
            self.skipTest(
                "secstorage.session.max global config is not set to 1 which means Multiple SSVM's are not present")
        elif testresult == 1:
            self.skipTest("only one SSVM is present")
        return
    ...
    def test_06_Browser_Upload_Volume_with_extended_file_extenstions(self):
        ...
        self.tready()
        self.debug(
            "========================= Test 37 Upload volume with extended file extenstions=========================")
        if self.uploadvolumeformat == "OVA":
            self.skipTest(
                "This test is need not be executed on VMWARE")
    ...
    def test_07_Browser_Upload_Volume_Storage_Cleanup_Config_Validation(self):
        ...
        self.tready()
        self.debug(
            "========================= Test 38 Validate storage.cleanup.enabled and storage.cleanup.interval ========================= ")
        config1 = Configurations.list(
            self.apiclient,
        ...
        if cleanup_enabled == "false":
            self.skipTest(
                "storage.cleanup.enabled is not set to true")

        if cleanup_interval > 600:
            self.skipTest(
                "storage.cleanup.interval is set to wait for more than 10 mins before cleanup. Please reduce the interval to less than 10 mins")
    ...
    def test_08_Browser_Upload_Volume_TamperedPostURL(self):
        ...
        self.tready()
        self.debug(
            "========================= Test 39 Upload Volume with tampered post URL=========================")
        invaliduploadvolume = self.invalidposturl()

        return

    @attr(tags=["advanced"], required_hardware="true")
    def test_09_Browser_Upload_Volume_PostURL_with_Deleted_Uploadvolume_Details(
            self):
        """
        Test Browser_Upload_Volume_PostURL_with_Deleted_Uploadvolume_Details
        """
        self.debug(
            "========================= Test 40 PostURL_with_Deleted_Upload_Abondaned volume details=========================")
        browse_up_vol = self.onlyupload()
    ...
    def test_10_Browser_Upload_Volume_API_with_imagepoolid(self):
        ...
        """
        self.debug(
            "========================= Test 41 Test Browser_Upload_Volume_API_with_imagepoolid=========================")
    ...
    def test_11_migrate_upload_volume(self):
        ...
        """
        self.debug(
            "========================= Test 42 Test Browser_Upload_Volume_Migration=========================")
        self.tready()
        browseup_vol = self.browse_upload_volume()
        vm1details = self.deploy_vm()
        self.attach_volume(vm1details, browseup_vol.id)
        ...
        self.debug(
            "========================= Test 43 Test VM Operations after Browser_Upload_Volume_Migration=========================")
        self.vmoperations(vm1details)

        self.debug(
            "========================= Test 44 Detach Browser_Upload_Volume after Migration and attach to a new VM=========================")
        self.detach_volume(vm1details, browseup_vol.id)
        vm2details = self.deploy_vm()
        self.attach_volume(vm2details, browseup_vol.id)
        self.vmoperations(vm2details)
        ...
        self.debug(
            "========================= Test 45 Detach Browser_Upload_Volume and Migrate to another storage=========================")

        self.detach_volume(vm2details, browseup_vol.id)
        self.volume_migration(browseup_vol, "None")

        self.debug(
            "========================= Test 46 Attach detached Browser_Upload_Volume after Migration =========================")

        self.attach_volume(vm2details, browseup_vol.id)
        self.vmoperations(vm2details)

        self.debug(
            "========================= Test 47 Detach  ,Resize,Attach  Browser_Upload_Volume after Migration =========================")
    ...
    def test_12_Browser_Upload_Volume_with_all_API_parameters(self):
        ...
        try:
            ...
            self.debug(
                "========================= Test 48 & 49 Upload volume with account name and domainid========================")

            browseup_vol1 = self.browse_upload_volume()

            self.debug(
                "========================= Test 50 Upload volume with projectid========================")
            browseup_vol2 = self.browse_upload_volume_with_projectid(
                self.project.id)

            self.debug(
                "========================= Test 51 Upload volume with out mandatory param zone id ========================")

            browseup_vol2 = self.browse_upload_volume_with_out_zoneid()

            self.debug(
                "========================= Test 52 Upload volume with out mandatory param format ========================")
    ...
    def test_13_Browser_Upload_Volume_volume_resource_limits(self):
        ...
        """
        self.debug(
            "========================= Test 53 Upload volume and verify volume limits========================")
    ...
    def test_14_Browser_Upload_Volume_secondary_storage_resource_limits(self):
        ...
        """
        self.debug(
            "========================= Test 54 Upload volume and verify secondary storage limits========================")
    ...
    def test_15_Browser_Upload_Volume_primary_storage_resource_limits(self):
        ...
        """
        self.debug(
            "========================= Test 55 Attach Upload volume and verify primary storage limits========================")
        self.tready()
        vm1details = self.deploy_vm()
        browseup_vol1 = self.browse_upload_volume()
        ...
        self.attach_volume(vm1details, browseup_vol1.id)

        afteruploadprimarystoragelimit = self.getstoragelimts(10)

        self.debug(afteruploadprimarystoragelimit)
        self.debug(initialprimarystoragelimit)
        self.debug(volumedetails[0].size)
        self.debug(initialprimarystoragelimit + volumedetails[0].size)
        if afteruploadprimarystoragelimit != (
                initialprimarystoragelimit +
                volumedetails[0].size):
            self.detach_volume(vm1details, browseup_vol1.id)
            cmd = deleteVolume.deleteVolumeCmd()
            cmd.id = browseup_vol1.id
            self.apiclient.deleteVolume(cmd)
    ...
    def test_16_Browser_Upload_volume_resource_limits_after_deletion(self):
        ...
        """
        self.debug(
            "========================= Test 56 Delete Upload volume and verify volume limits========================")
        browseup_vol1 = self.browse_upload_volume()
        initialvolumelimit = self.getvolumelimts()
        ...
        if aftervolumelimit != (initialvolumelimit - 1):
            self.fail(
                "Volume Resource Count is not updated after deletion")

        return

    @attr(tags=["advanced"], required_hardware="true")
    def test_17_Browser_Upload_Volume_secondary_storage_resource_limits_after_deletion(
            self):
        """
        Test Browser_Upload_Volume secondary_storage_resource_limits_after_deletion
        """
        self.debug(
            "========================= Test 57 Delete Upload volume and secondary storage limits========================")
        ...
        if afteruploadsecondarystoragelimit != (
                initialuploadsecondarystoragelimit -
                volumedetails[0].size):
            self.fail(
                "Secondary Storage Resouce Count is not updated after deletion")

        return

    @attr(tags=["advanced"], required_hardware="true")
    def test_18_Browser_Upload_Volume_with_invalid_signature(self):
        """
        Test Browser_Upload_Volume_with_invalid_signature
        """
        self.debug(
            "========================= Test 58 Upload volume with invalid signature========================")
        self.invalidsignatureupload()
        return
    ...
    def tearDownClass(self):
        try:
            self.apiclient = super(
                TestBrowseUploadVolume, self).getClsTestClient().getApiClient()
