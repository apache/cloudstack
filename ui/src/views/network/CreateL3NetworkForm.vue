...
export default {
  ...
  methods: {
    ...
    validateAddressFamilies (values) {
      const anyIpv4 = ['cidr', 'startip', 'endip'].some(key => values[key])
      if (anyIpv4 && !values.cidr) {
        return this.$t('message.error.l3.ipv4.incomplete')
      }
      if (!anyIpv4 && !values.ip6cidr) {
        return this.$t('message.error.l3.no.address.family')
      }
      return null
    },
    handleSubmit (e) {
      ...
      this.formRef.value.validate().then(() => {
        ...
        const familyError = this.validateAddressFamilies(values)
        if (familyError) {
        ...
        for (const key of ['cidr', 'startip', 'endip', 'ip6cidr', 'dns1', 'dns2']) {
          if (values[key]) {
