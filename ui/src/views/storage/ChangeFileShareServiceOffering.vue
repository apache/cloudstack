       ...
       >
        <a-form-item>
          <a-alert type="warning">
            <template #message>
             <span v-html="$t('message.confirm.change.service.offering.for.fileshare')" />
            </template>
         </a-alert>
        </a-form-item>
...
export default {
  ...
  beforeCreate () {
    this.apiParams = this.$getApiParams('changeFileShareServiceOffering')
  },
  created () {
    this.initForm()
    this.fetchData()
  ...
  methods: {
    ...
    handleSubmit (e) {
      ...
      this.formRef.value.validate().then(async () => {
        ...
        this.loading = true
        api('changeFileShareServiceOffering', data).then(response => {
          this.$pollJob({
            jobId: response.changefileshareserviceofferingresponse.jobid,
            title: this.$t('label.change.service.offering'),
            description: values.name,
            successMessage: this.$t('message.success.change.offering'),
            errorMessage: this.$t('message.change.service.offering.fileshare.failed'),
            loadingMessage: this.$t('message.change.service.offering.fileshare.processing'),
