...
(function ($, cloudStack) {
    var elems = {
        aclDialog: function (args) {
            ...
            var $acl = $('<div>').addClass('acl').multiEdit($.extend(true, {}, actionArgs.multiEdit, {
                context: context
            }));
            // Show ACL dialog
            if (isDialog) {
        ...
        vpcConfigureTooltip: function (args) {
            ...
            var $tooltip = $('<div>').addClass('vpc-configure-tooltip').append($('<div>').addClass('arrow'));
                        ...
                        case 'network-acls':
                            $browser.cloudBrowser('addPanel', {
                                ...
                                complete: function ($panel) {
                                    $panel.listView($.extend(true, {}, acl.listView, {
                                        listView: {
                                            actions: {
                                                add: {
                                        ...
                                        addAction.action({
                                            ...
                                            response: {
                                                success: function (args) {
                                                    ...
                                                    cloudStack.ui.notifications.add(notification, success, {}, error, {});
                                                },
                                                error: error
                                            }
                                        });
        ...
        tier: function (args) {
            ...
            var actions = $.map(args.actions ? args.actions : {}, function (value, key) {
                return {
                    id: key,
                    action: value
                };
            ...
            $vmCount.append($('<div>').addClass('loading-overlay')
            ...
            if (isPlaceholder) {
                ...
                $title.html(cloudStack.concat(name, 8));
                $title.attr('title', name);
                $cidr.html(cidr);
                $vmCount.append($('<span>').addClass('total').html(virtualMachines != null ? virtualMachines.length : 0), _l('label.vms'));
                $tier.append($actions);
        ...
        chart: function (args) {
            ...
            if (!isAdmin())
                $router.addClass('disabled');
            var $chart = $('<div>').addClass('vpc-chart');
            var $title = $('<div>').addClass('vpc-title')
                .append($('<span>').html(vpcName))
                .append(elems.vpcConfigureArea({
                context: context,
                $browser: $browser,
                $chart: $chart,
